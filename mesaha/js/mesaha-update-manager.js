...
(function(){
  ...
  if(window.MESAHA_SUITE_MODE){
    ...
    async function suiteRepair(){var reg=await navigator.serviceWorker.register('../service-worker.js?v=19',{scope:'../',updateViaCache:'none'});await navigator.serviceWorker.ready;try{(reg.active||navigator.serviceWorker.controller).postMessage({type:'CACHE_ALL'})}catch(e){}return{ok:true,repaired:true,preserved:true,ready:true,build:18,integrity:'suite-v19'}}
    var suiteApi={apiVersion:547,repair:suiteRepair,clearOldCaches:async function(){return[]},updateAndReload:async function(){location.href='../';return{ok:true,redirecting:true}},executeUpdatePage:suiteRepair,installLatest:suiteRepair,check:async function(){return{remote:{version:'suite-v19',build:18},newer:false,unavailable:!navigator.onLine}},fetchRemote:async function(){return{version:'suite-v19',build:18}},flushData:async function(){},workerStatus:async function(){return{ready:true,build:18,integrity:'suite-v19',criticalCount:99}},verifiedStatus:function(st){return!!(st&&st.ready)}};

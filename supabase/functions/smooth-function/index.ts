...
const DRIVE_SCRIPT_URL = Deno.env.get("MESAHA_DRIVE_SCRIPT_URL") || "https://script.google.com/macros/s/AKfycbzOYh2MyOQmwVQh-7Jm9KyjaFjmjSwgHZSw7XKAVzDS1ibmcM5bQZVYdn-NyesI-ph7/exec";
const DRIVE_SECRET = Deno.env.get("MESAHA_DRIVE_SECRET") || "MESAHAYEDEK_2026_YAKUP_43";
  ...
  for (const action of actions) {
    try {
      const res = await fetch(DRIVE_SCRIPT_URL, {
        ...
        body: JSON.stringify({ secret: DRIVE_SECRET, action, fileId: id, permanent: true }),
        redirect: "follow",
      });
      const text = await res.text();
      let data: any = {};
      try { data = text ? JSON.parse(text) : {}; } catch { data = {}; }
      if (res.ok && data && data.ok !== false) return { ok: true, id, action, response: data };
      lastError = clean(data?.error || data?.message || `Drive ${action} HTTP ${res.status}`);
    } catch (e) {
...
async function seflikFolderContext(req: Request, body: JsonObj) {
  const ipInfo = clientIpInfo(req, body);
  const userId = await authUserId(req);
  if (!userId) return { response: json({ ok: false, error: "Şeflik Klasörü için kullanıcı oturumu bulunamadı" }, 401) };
  const profile = profileValues(body, userId, ipInfo);
  if (!hasIdentityForAdmin(profile)) return { response: json({ ok: false, error: "Geçerli kullanıcı adı ve şeflik gerekli" }, 400) };
  const block = await anyBlock({ user_id: userId, user_key: clean(profile.user_key), ip: clean(ipInfo.ip), device_id: clean(profile.device_id) });
  if (block) return { response: json({ ok: false, blocked: true, reason: first(block.reason, "Yönetici engeli") }, 403) };
  const seflik = clean(profile.seflik);
  ...
  await upsertUsageCurrent(profile, { ...body, edgeIp: ipInfo, source: "seflik-folder" });
  return { userId, profile, ipInfo, seflik, seflikKey, userKey: clean(profile.user_key) };
}
...
async function seflikFolderPush(req: Request, body: JsonObj) {
  ...
  const input = arr(body.records).map(obj).slice(0, 250);

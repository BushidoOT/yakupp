...
async function seflikFolderListMySefliks(req: Request, body: JsonObj) {
  ...
  /* Kurucu üyeliği eski veri nedeniyle eksikse yine göster. */
  ...
  for (const fRaw of created.items || []) {
    ...
    if (!key || seen.has(key)) continue;
    seen.add(key); out.push(folderView(f, { role: 'owner' }, ctx));
  }
  return json({ ok: true, folders: out.sort((a,b)=>clean(a.seflik).localeCompare(clean(b.seflik), 'tr')), server_time: nowIso() });
}
...
async function seflikFolderListMembers(req: Request, body: JsonObj) {
  const ctx: any = await seflikFolderContext(req, body);
  if (ctx.response) return ctx.response;
  const { data, error } = await admin.from(TABLES.seflikMembers)
  ...
  if (error) return json({ ok: false, error: error.message }, 500);
  const creator = clean(ctx.folder.created_by_user_id) === ctx.userId;
  const members = arr(data).map((x) => {
    const m = obj(x);
    return {
...
async function seflikFolderContext(req: Request, body: JsonObj) {
  ...
  const folder = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: seflikKey }));
  if (!folder.id || clean(folder.status || 'active') !== 'active') return { response: json({ ok: false, error: "Bu şeflik klasörü oluşturulmamış. Önce Şeflik Oluştur kullanın." }, 404) };
  const member = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: seflikKey, user_id: ctx.userId }));
  const isCreator = clean(folder.created_by_user_id) === ctx.userId;

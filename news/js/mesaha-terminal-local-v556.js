...
(function(){
  ...
  var BLOCK_SELECTORS=[
    '#cloudBackupBtnV316','#cloudRestoreBtnV316','#seflikSendFromRecordsV529',
    '#seflikFolderHomeShortcutV528','#seflikFolderCreateV529','#seflikFolderSendV528','#seflikFolderSyncV528',
    '[data-nav="seflikFolder"]','[data-hybrid-id-v501]','[data-hybrid-delete-id-v506]',
    '[data-drive-restore-v463]','[data-seflik-continue]','[data-seflik-delete]'
  ].join(',');
  ...
  function applyBadge(){
    ...
    var sync=$('panelSyncTextV316');if(sync)sync.textContent='Terminal modu: yerel kullanım • bulut kapalı';
  }
  function labelBlockedButtons(){
    if(!terminal())return;
    [['cloudBackupBtnV316','Google ile giriş yap'],['cloudRestoreBtnV316','Google ile giriş yap'],['seflikSendFromRecordsV529','Google ile giriş yap'],['seflikFolderCreateV529','Google gerekli'],['seflikFolderSendV528','Google gerekli'],['seflikFolderSyncV528','Google gerekli']].forEach(function(x){var b=$(x[0]);if(b){b.textContent=x[1];b.title='Terminal modunda bulut kapalı. Kullanmak için Google ile giriş yapın.'}});
    var st=$('seflikFolderStatusV528');if(st)st.textContent='Terminal modunda Şeflik Klasörü kapalı. Kullanmak için Google ile giriş yapın.';
  ...
  window.MesahaTerminalLocalV556={isActive:terminal,disable:clearTerminal,google:goGoogle,boot:boot};
})();

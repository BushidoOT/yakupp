/* Mesaha İO V5.51 — mevcut cihaz geçişi, user_id tabanlı kesin engel ve güvenli Edge/anti-spam bulut motoru
...
(function(){
  ...
  async function guardCheck(action,user,extra){
    ...
    try{
      return await api.edge(action||'profile_ping',Object.assign({userKey:userKey(user.name,user.seflik),name:user.name,seflik:user.seflik,bolmeNo:user.bolmeNo,deviceId:getDeviceId(),appVersion:versionText(),source:'mesaha-web-v551'},extra||{}));
    }catch(e){
      if(isBlockedError(e)){
        e.blocked=true;

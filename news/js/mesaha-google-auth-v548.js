/* Mesaha İO V5.61 — Google OAuth + terminal kod eşleştirme + panel üstü modal ve misafir terminal modu.
...
(function(){
  ...
  async function claimTerminalCode(codeArg){var el=document.getElementById('terminalPairCodeV557'),el2=document.getElementById('terminalPairCodePanelV561'),code=clean(codeArg||el&&el.value||el2&&el2.value).toUpperCase();if(code.length<6)throw new Error('Telefondan oluşturulan terminal kodunu girin.');loading('Terminal kodu kontrol ediliyor…');loginLog('terminal_code_claim_start',{codeLength:code.length,source:codeArg?'panel':'overlay'},'info');var out=await anonRpc('mesaha_claim_terminal_code_v557',{p_code:code,p_device_info:deviceInfo(),p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'V5.61'});var t=out.terminal||out;writeTerminalIdentity({name:t.name,seflik:t.seflik,bolmeNo:t.bolme_no||t.bolmeNo,terminalCode:code,terminalToken:t.terminal_token||t.token,pairedUserId:t.owner_user_id,pairedEmail:t.owner_email,expiresAt:t.expires_at,pairedAt:new Date().toISOString(),source:'pair_code'});hide();try{if(typeof window.mesahaFloatToastV315==='function')window.mesahaFloatToastV315('Terminal eşleşti',(t.name||'Kullanıcı')+' • '+(t.seflik||'Şeflik'),'success');else if(typeof window.toast==='function')window.toast('Terminal eşleşti',(t.name||'Kullanıcı')+' • '+(t.seflik||'Şeflik'),'success')}catch(e){}try{if(window.MesahaTerminalLocalV556&&typeof window.MesahaTerminalLocalV556.boot==='function')window.MesahaTerminalLocalV556.boot()}catch(e){}return true}

/* Mesaha İO V5.57 — Google OAuth + terminal kod eşleştirme.
   - Google hesabı doğrulandıktan sonra kullanıcı otomatik onaylanır.
   - Kullanıcı panelinden terminal kodu üretilebilir.
   - Terminal cihaz Google hesabı açmadan kod ile kullanıcı/şeflik kimliğine eşleşir; bulut yine kapalı kalır. */
(function(){
  'use strict';
  ...
  async function anonRpc(name,params){
    ...
    if(!res.ok||out.ok===false){var msg=clean(out.message||out.error||out.details||out.hint||('Terminal kodu doğrulanamadı: '+res.status));loginLog('terminal_rpc_error',{name:name,status:res.status,message:msg,out:out},'error');throw new Error(msg)}
  ...
  function writeTerminalIdentity(data){
    ...
    var term={active:true,name:name,seflik:seflik,bolmeNo:bolme,createdAt:new Date().toISOString(),version:'v557',source:data.source||'manual'};
    if(data.terminalCode)term.terminalCode=clean(data.terminalCode);if(data.pairedUserId)term.pairedUserId=clean(data.pairedUserId);if(data.pairedEmail)term.pairedEmail=clean(data.pairedEmail);if(data.expiresAt)term.expiresAt=data.expiresAt;if(data.pairedAt)term.pairedAt=data.pairedAt;
  ...
  function terminalForm(){var li=terminalData();show('<div class="google-auth-status-v548 pending"><b>Terminal giriş modu</b><br>Telefondan Google ile giriş yapan kullanıcı, kullanıcı panelinden terminal kodu oluşturabilir. Bu kodla terminal kimliği eşleşir; bulut özellikleri yine Google girişine bağlı kalır.</div><div class="google-auth-fields-v548"><label>Terminal Kodu<input id="terminalPairCodeV557" maxlength="20" inputmode="text" placeholder="Örn: A1B2-C3D4" autocomplete="one-time-code"></label></div>'+button('terminal-claim','Kod ile terminali eşleştir','green')+'<p class="google-auth-note-v548">Kod yoksa aşağıdan elle yerel terminal modu açılabilir.</p><div class="google-auth-fields-v548"><label>Kullanıcı adı<input id="terminalNameV556" maxlength="120" value="'+esc(li.name)+'" autocomplete="name"></label><label>Şeflik<input id="terminalSeflikV556" maxlength="120" value="'+esc(li.seflik)+'" autocomplete="organization"></label><label>Bölme No <input id="terminalBolmeV556" maxlength="80" value="'+esc(li.bolmeNo)+'"></label></div>'+button('terminal-save','Elle terminal modunda devam et','subtle')+button('google','Google ile giriş yap','primary')+'<p class="google-auth-note-v548">Terminal modunda kayıtlar cihazda kalır. Bulut özelliği gerekirse Google ile giriş yapmanız istenir.</p>')}
  async function claimTerminalCode(){var el=document.getElementById('terminalPairCodeV557'),code=clean(el&&el.value).toUpperCase();if(code.length<6)throw new Error('Telefondan oluşturulan terminal kodunu girin.');loading('Terminal kodu kontrol ediliyor…');loginLog('terminal_code_claim_start',{codeLength:code.length},'info');var out=await anonRpc('mesaha_claim_terminal_code_v557',{p_code:code,p_device_info:deviceInfo(),p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'V5.57'});var t=out.terminal||out;writeTerminalIdentity({name:t.name,seflik:t.seflik,bolmeNo:t.bolme_no||t.bolmeNo,terminalCode:code,pairedUserId:t.owner_user_id,pairedEmail:t.owner_email,expiresAt:t.expires_at,pairedAt:new Date().toISOString(),source:'pair_code'});hide();try{if(typeof window.mesahaFloatToastV315==='function')window.mesahaFloatToastV315('Terminal eşleşti',(t.name||'Kullanıcı')+' • '+(t.seflik||'Şeflik'),'success');else if(typeof window.toast==='function')window.toast('Terminal eşleşti',(t.name||'Kullanıcı')+' • '+(t.seflik||'Şeflik'),'success')}catch(e){}return true}
...
#firstLoginOverlayV321{display:none!important}.google-auth-v548{position:fixed;inset:0;z-index:100500;background:linear-gradient(155deg,rgba(240,253,244,.98),rgba(239,246,255,.98));display:flex;align-items:center;justify-content:center;padding:18px;font-family:inherit}.google-auth-v548.hidden{display:none!important}.google-auth-card-v548{width:min(460px,100%);max-height:calc(100dvh - 36px);overflow:auto;background:#fff;border:1px solid #dbeafe;border-radius:28px;padding:24px;box-shadow:0 30px 90px rgba(15,23,42,.22);color:#0f172a}.google-auth-brand-v548{display:flex;gap:13px;align-items:center;margin-bottom:17px}.google-auth-brand-v548 img{width:58px;height:58px;object-fit:contain}.google-auth-brand-v548 h2{font-size:23px;margin:0}.google-auth-brand-v548 p{margin:4px 0 0;color:#64748b;font-weight:700;font-size:13px}.google-auth-status-v548{border-radius:18px;padding:14px;margin:12px 0;background:#f8fafc;border:1px solid #e2e8f0;line-height:1.45;font-weight:750;font-size:14px}.google-auth-status-v548.pending{background:#fffbeb;border-color:#fde68a;color:#92400e}.google-auth-status-v548.denied{background:#fef2f2;border-color:#fecaca;color:#991b1b}.google-auth-status-v548.ok{background:#f0fdf4;border-color:#bbf7d0;color:#166534}.google-auth-btn-v548{width:100%;min-height:54px;border:0;border-radius:17px;padding:12px 15px;font-size:16px;font-weight:950;margin-top:10px;cursor:pointer}.google-auth-btn-v548.primary{background:#fff;color:#1f2937;border:1px solid #cbd5e1;box-shadow:0 8px 22px rgba(15,23,42,.09)}.google-auth-btn-v548.primary:before{content:"G";display:inline-grid;place-items:center;width:27px;height:27px;margin-right:10px;border-radius:50%;font-size:18px;color:#2563eb;background:#eff6ff}.google-auth-btn-v548.green{background:#16a34a;color:#fff}.google-auth-btn-v548.subtle{background:#f1f5f9;color:#334155}.google-auth-btn-v548.danger{background:#fff1f2;color:#be123c}.google-auth-btn-v548:disabled{opacity:.55;cursor:wait}.google-auth-fields-v548{display:grid;gap:11px;margin:14px 0}.google-auth-fields-v548 label{font-weight:900;font-size:13px}.google-auth-fields-v548 input{box-sizing:border-box;width:100%;margin-top:6px;min-height:50px;border:1px solid #cbd5e1;border-radius:15px;padding:12px;font:inherit;font-weight:800}.google-auth-email-v548{overflow-wrap:anywhere;font-size:12px;color:#475569;text-align:center;margin:10px 0}.google-auth-note-v548{font-size:12px;color:#64748b;line-height:1.45;margin:13px 0 0}.terminal-code-v557{font-size:34px;font-weight:1000;letter-spacing:3px;text-align:center;border:1px dashed #93c5fd;border-radius:18px;background:#eff6ff;color:#1d4ed8;padding:18px;margin:12px 0}.terminal-code-panel-v557{border:1px solid #bfdbfe;background:#eff6ff;border-radius:18px;padding:12px;margin:12px 0;color:#1e3a8a}.terminal-code-panel-v557 b{font-size:15px}.terminal-code-panel-v557 p{font-size:12px;line-height:1.45;margin:6px 0 10px;color:#475569;font-weight:750}.google-auth-spinner-v548{width:24px;height:24px;border:3px solid #dbeafe;border-top-color:#2563eb;border-radius:50%;animation:googleSpinV548 .8s linear infinite;margin:8px auto}@keyframes googleSpinV548{to{transform:rotate(360deg)}}';document.head.appendChild(s)}
  ...
  async function accessStatus(force){
    ...
    statusPromise=(async function(){try{var out=await rpc('mesaha_google_access_status_v557',{p_device_info:deviceInfo(),p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'V5.57'});var x=out.access||out;x.user_id=x.user_id||uid();x.email=x.email||clean(user().email);clearEmailExistsRetry();cacheAccess(x);loginLog('access_status_ok',{status:x.status,email:x.email,user_id:x.user_id},'info');return x}catch(e){loginLog('access_status_error',{error:e,message:e&&e.message},'error');var c=currentAccess||cached();if(c&&c.status==='approved')return c;throw e}finally{statusPromise=null}})();
  ...
  function terminalCodeHtml(code,expiresAt){return '<div class="google-auth-status-v548 ok"><b>Terminal kodu hazır</b><br>Bu kodu terminal cihazdaki <b>Terminal giriş modu</b> ekranına girin.</div><div class="terminal-code-v557" id="terminalCodeValueV557">'+esc(code)+'</div><p class="google-auth-note-v548">Kod tek kullanımlıktır ve '+esc(expiresAt||'kısa süre')+' tarihine kadar geçerlidir. Terminal bu kodla kullanıcı/şeflik kimliğine eşleşir; bulut özellikleri yine kapalı kalır.</p>'+button('terminal-code-copy','Kodu kopyala','primary')+button('terminal-code-close','Kapat','subtle')}
  async function createTerminalCode(){var x=currentAccess||cached();if(!x||clean(x.status)!=='approved')throw new Error('Terminal kodu oluşturmak için önce Google ile giriş yapın.');loading('Terminal kodu oluşturuluyor…');loginLog('terminal_code_create_start',{userId:uid()},'info');var out=await rpc('mesaha_create_terminal_code_v557',{p_label:'terminal',p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'V5.57'});var t=out.terminal||out;lastTerminalCode=clean(t.code);loginLog('terminal_code_create_ok',{expires_at:t.expires_at},'info');show(terminalCodeHtml(lastTerminalCode,t.expires_at));return out}
  ...
  function renderAccess(x){
    ...
    if(status==='pending')return show('<div class="google-auth-status-v548 pending"><b>Otomatik onay bekleniyor</b><br>Bu kullanıcı eski SQL nedeniyle beklemede görünüyor. Supabase SQL Editor’da V5.57 terminal kodu ve otomatik Google onay SQL dosyasını çalıştırın.</div><div class="google-auth-email-v548">'+esc(email)+'</div>'+button('refresh','Tekrar kontrol et','primary')+button('request-form','Bilgilerle tekrar dene','green')+button('logout','Başka Google hesabı kullan','subtle'));
    ...
    show('<div class="google-auth-status-v548"><b>Google hesabın doğrulandı.</b><br>Kullanıcı adı ve şeflik girilince hesap otomatik tanımlanacak; admin onayı beklemeyecek.</div><div class="google-auth-email-v548">'+esc(email)+'</div><div class="google-auth-fields-v548"><label>Kullanıcı adı<input id="googleRequestNameV548" maxlength="120" value="'+esc(li.name)+'" autocomplete="name"></label><label>Şeflik<input id="googleRequestSeflikV548" maxlength="120" value="'+esc(li.seflik)+'" autocomplete="organization"></label></div>'+button('request','Google ile devam et','green')+button('logout','Başka Google hesabı kullan','subtle')+'<p class="google-auth-note-v548">Bu bilgiler doğrulanmış Google hesabına otomatik bağlanır.</p>')
  }
  async function requestAccess(data){loginLog('request_access_click',{auto:!!data},'info');data=data||{};var n=document.getElementById('googleRequestNameV548'),s=document.getElementById('googleRequestSeflikV548'),name=clean(data.name||n&&n.value),seflik=clean(data.seflik||s&&s.value);if(!validIdentity(name,seflik))throw new Error('Geçerli kullanıcı adı ve şeflik girin.');loading('Google hesabı kullanıcıya tanımlanıyor…');loginLog('request_access_send',{name:name,seflik:seflik,auto:true},'info');var out=await rpc('mesaha_google_access_request_v557',{p_name:name,p_seflik:seflik,p_device_info:deviceInfo(),p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'V5.57'});renderAccess(out.access||out)}
  ...
  async function handle(action){if(action==='google'){clearTerminalMode();return beginGoogle()}if(action==='terminal')return terminalForm();if(action==='terminal-save')return saveTerminalFromForm();if(action==='terminal-claim')return claimTerminalCode();if(action==='terminal-code-copy')return copyText(lastTerminalCode).then(function(){try{if(typeof window.mesahaFloatToastV315==='function')window.mesahaFloatToastV315('Kod kopyalandı',lastTerminalCode,'success')}catch(e){}});if(action==='terminal-code-close'){hide();ensureTerminalCodePanel();return true}if(action==='refresh')return boot(true);if(action==='request')return requestAccess();if(action==='request-form')return renderAccess({status:'unregistered',email:user().email});if(action==='logout')return logout()}
    ...
    if(!session()){loginLog('boot_no_session',{terminalAvailable:true},'info');return show('<div class="google-auth-status-v548"><b>Google ile giriş gerekli</b><br>Bulut ve ortak şeflik özellikleri için Google hesabı gerekir.</div>'+button('google','Google ile giriş yap','primary')+button('terminal','Terminal giriş modu','subtle')+'<p class="google-auth-note-v548">Terminal koduyla kullanıcıya eşleşebilir; Bulut, Drive ve Şeflik Klasörü kapalı olur.</p>');}
  ...
  window.MesahaGoogleAuthV548={boot:boot,status:accessStatus,access:function(){return currentAccess||cached()},isApproved:function(){var x=currentAccess||cached();return !!(x&&x.status==='approved')},createTerminalCode:createTerminalCode,logout:logout};

/* Mesaha İO V5.71 — Google Auth + güvenli Supabase uyumluluk motoru
...
(function(){
  ...
  function rowFromData(table,id,data,parent){
    ...
    if(table==='mesaha_backup_slots'){
      ...
      var owner=data.user||p.user||data, ownerName=clean(data.googleFullName||data.displayName||owner.displayName||owner.googleFullName||googlePanelNameV560()||data.name||owner.name), ownerSeflik=clean(data.seflik||owner.seflik), ownerKey=clean(data.userKey||data.user_key||owner.userKey||owner.user_key);
      if(!ownerKey&&ownerName&&ownerSeflik)ownerKey=clean(ownerName+'__'+ownerSeflik).toLocaleLowerCase('tr-TR').replace(/[ç]/g,'c').replace(/[ğ]/g,'g').replace(/[ı]/g,'i').replace(/[ö]/g,'o').replace(/[ş]/g,'s').replace(/[ü]/g,'u').replace(/[^a-z0-9_-]+/g,'_').slice(0,120);
      return {user_id:userId,slot_id:slot,user_key:ownerKey||null,name:ownerName||null,seflik:ownerSeflik||null,backup_name:name,record_count:Number(data.recordCount||data.count||0)||0,total_volume:Number(data.totalVolume||data.m3||0)||0,payload:p,archived:data.archived===true};
    }

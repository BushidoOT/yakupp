(function (root) {
  ...
  async function loadFolders() {
    ...
    var list = Array.isArray(out && out.folders) ? out.folders : [];
    write(K.folders, list);
    return list;
  }
  ...
  async function rpc(name, params) {
    ...
    var response = await fetch(base + "/rest/v1/rpc/" + encodeURIComponent(name), { method: "POST", cache: "no-store", headers: { "Content-Type": "application/json", apikey: anon, Authorization: "Bearer " + clean(current.access_token) }, body: JSON.stringify(params || {}) });

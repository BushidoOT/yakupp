...
';document.head.appendChild(st)}
  ...
  function renderFolders(list,msg){ensureStyles();var host=document.querySelector('#seflikFolderView .seflik-folder-control');if(!host)return;list=mergeFallbackFolder(list||[]);var box=$('seflikGovernanceV564');if(!box){box=document.createElement('section');box.id='seflikGovernanceV564';box.className='seflik-v564-box';host.parentNode.insertBefore(box,host)}var activeName=activeSeflik();var loadingNote=msg?'<div class="seflik-v564-note '+(/yükleniyor/i.test(String(msg))?'ok':'')+'">'+esc(msg)+'</div>':'';box.innerHTML='<h3>Şeflik Seçimi</h3><p>Şeflik, ormancı ve bölme yönetimi Yönetim bölümünden yapılır. Burada yalnızca aktif şefliği seçebilirsiniz.</p><div class="seflik-v564-grid"><div class="seflik-v564-row"><select id="seflikSelectV564"><option value="">Şeflik seçin</option>'+((list||[]).map(function(f){var local=f.is_local?' • yerel':'';return '<option value="'+esc(f.seflik)+'" '+(sameSeflik(f.seflik,activeName)?'selected':'')+'>'+esc(f.seflik)+(f.is_creator?' • kurucu':local||' • üye')+'</option>'}).join(''))+'</select></div>'+loadingNote+'<div class="seflik-v564-note ok">Düzenleme yapmak için Yönetim bölümünü açın.</div></div>';bindBox()}

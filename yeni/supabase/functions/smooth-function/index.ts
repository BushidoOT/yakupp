...
async function adminHideDriveBackup(req: Request, body: JsonObj) {
  ...
  return json({ ok: true, supabaseDeletes: results, hidden: payload, hidden_drive_ids: [drive_file_id] });
}
...
async function adminDeleteUser(req: Request, body: JsonObj) {
  ...
  if (u.user_id) {
    for (const table of [TABLES.backupChunks, TABLES.backupSlots, TABLES.usageDaily, TABLES.usageCurrent, TABLES.logs, TABLES.events, TABLES.profiles, TABLES.seflikSyncs]) {

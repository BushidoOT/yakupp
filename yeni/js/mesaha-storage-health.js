(function(){
  'use strict';
  if(window.MesahaStorageHealth && window.MesahaStorageHealth.__v449) return;
  ...
  function check(showAlert){var x=info(); if(showAlert) alert(message(x)); if(x.warning) toast('Depolama büyümüş; yedek almanız önerilir.'); return x;}
  ...
  window.addEventListener('mesaha:records-saved',function(){setTimeout(function(){check(false);},300);},{passive:true});
  ...
  window.MesahaStorageHealth={__v449:true,info:info,check:check,download:download};
})();

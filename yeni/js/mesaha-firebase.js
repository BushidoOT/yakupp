/* Mesaha İO v415 — sağlam Firebase motoru
...
(function(){
  ...
  async function health(){
    const r = await ready();
    try{
      await withTimeout(r.db.collection('healthChecks').doc('client').set({ok:true, atMs:Date.now(), appVersion:(window.APP_VERSION||'V4.00'), source:'client'}, {merge:true}), 12000, 'Firebase test');
    }catch(e){
      readyPromise = null;

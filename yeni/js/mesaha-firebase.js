...
(function(){
  'use strict';
  var VERSION='v455_offline_filtre_log_temizlik';
  ...
  async function health(){var r=await ready();await r.db.collection('healthChecks').doc('client').set({ok:true,atMs:Date.now(),appVersion:(window.APP_VERSION||'V4.40'),source:'supabase'},{merge:true});lastOkMs=Date.now();lastError='';return r;}

/* Mesaha İO V5.29 — Şeflik Klasörü, açık bölmeler ve mesahaya devam */
(function(){
  'use strict';
  if(window.MesahaSeflikFolderV529) return;

  var EDGE_URL='https://swrbpdpotmirnmtqnuba.supabase.co/functions/v1/smooth-function';
  var DRIVE_URL='https://script.google.com/macros/s/AKfycbzOYh2MyOQmwVQh-7Jm9KyjaFjmjSwgHZSw7XKAVzDS1ibmcM5bQZVYdn-NyesI-ph7/exec';
  var DRIVE_SECRET='MESAHAYEDEK_2026_YAKUP_43';
  ...
  function user(){var p=readJson(PANEL_KEY,{}),s=settings();return{name:clean(p.name||s.ekipNot),seflik:clean(p.seflik||s.seflik),bolmeNo:clean(p.bolmeNo||s.bolmeNo)}}
  function validIdentity(u){if(window.MesahaRuntimeV527&&typeof window.MesahaRuntimeV527.validIdentity==='function')return window.MesahaRuntimeV527.validIdentity(u.name,u.seflik);return clean(u.name).length>1&&clean(u.seflik).length>1}
  ...
  async function edge(action,payload){
    ...
    var body=Object.assign({action:action,name:u.name,seflik:u.seflik,bolmeNo:u.bolmeNo,deviceId:info.deviceId,deviceInfo:info,appVersion:info.appVersion,fileVersion:info.fileVersion,source:'mesaha-seflik-folder-v529'},payload||{});
    var ctrl=typeof AbortController!=='undefined'?new AbortController():null;
    var timer=ctrl?setTimeout(function(){ctrl.abort()},35000):0;
    try{
      var res=await fetch(EDGE_URL,{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+t,apikey:anonKey()},body:JSON.stringify(body),cache:'no-store',signal:ctrl&&ctrl.signal});
      var txt=await res.text(),j={};try{j=txt?JSON.parse(txt):{}}catch(e){throw new Error('Sunucu cevabı okunamadı')}
      if(!res.ok||!j.ok)throw new Error(j.error||j.reason||('Sunucu hatası '+res.status));
      return j;
    }finally{if(timer)clearTimeout(timer)}
  }

  function openDivisions(){return divisions.filter(function(x){return clean(x.status||'open')==='open'})}
  function selectedBolme(){return clean(($('seflikFolderBolmeV528')||{}).value)}
  function currentLocalList(){return records().slice()}
  function updatePreview(){
    var bolme=selectedBolme(),list=currentLocalList(),m3=list.reduce(function(a,r){return a+volume(r)},0);
    var el=$('seflikFolderLocalPreviewV528');
    if(el)el.textContent=bolme?(list.length.toLocaleString('tr-TR')+' kayıt • '+fmt(m3,3)+' m³ • Bölme '+bolme):'Önce açık bir bölme seçin';
    var btn=$('seflikFolderSendV528');if(btn)btn.disabled=busy||!bolme||!list.length||!openDivisions().some(function(x){return clean(x.bolme_no)===bolme});
  }
  function fillIdentity(){
    var u=user(),name=$('seflikFolderIdentityV528');if(name)name.textContent=u.seflik||'Şeflik seçilmedi';
  ...
  function renderList(){
    ...
    if(!list.length){box.innerHTML='<div class="seflik-folder-empty">Henüz açık bölme oluşturulmamış.<br><small>“Bölme Oluştur” ile ilk ortak çalışma alanını açın.</small></div>';return}
    box.innerHTML=list.map(function(x){var contributors=(x.contributors||[]).filter(Boolean),drive=!!x.drive_backed_up,empty=num(x.record_count)===0;return '<article class="seflik-division-card">'+
      '<div class="seflik-division-top"><div class="seflik-division-title"><span class="seflik-division-icon">📁</span><div><b>Bölme '+esc(x.bolme_no||'-')+'</b><small>'+esc(contributors.length?contributors.join(', '):(x.created_by_name?('Oluşturan: '+x.created_by_name):'Henüz mesaha gönderilmedi'))+'</small></div></div><span class="seflik-open-pill">Açık</span></div>'+
      ...
      '<div class="seflik-division-actions seflik-division-actions-v529"><button class="btn soft" type="button" data-seflik-read="'+esc(x.bolme_no)+'">Kayıtları Gör</button><button class="btn primary" type="button" data-seflik-continue="'+esc(x.bolme_no)+'">'+(empty?'Mesahaya Başla':'Mesahaya Devam Et')+'</button></div></article>'}).join('');
  }

  async function syncFolder(showToast){
    var u=user();if(!validIdentity(u)){notify('Önce kullanıcı girişi yapın','Şeflik Klasörü için kullanıcı adı ve şeflik zorunlu.','warning');return false}
    if(!navigator.onLine){var cached=readJson(CACHE_KEY,null);if(cached&&Array.isArray(cached.divisions)){divisions=cached.divisions;renderList();setStatus('Çevrimdışı: son senkronize edilen klasör gösteriliyor.','');return true}throw new Error('Senkronizasyon için internet gerekli')}
    setStatus('Şeflik klasörü senkronize ediliyor…','busy');
    var out=await edge('seflik_folder_list',{});divisions=Array.isArray(out.divisions)?out.divisions:(Array.isArray(out.summaries)?out.summaries:[]);writeJson(CACHE_KEY,{at:new Date().toISOString(),seflik:u.seflik,divisions:divisions});renderList();setStatus('Senkronizasyon tamamlandı • '+new Date().toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'}),'success');if(showToast)notify('Şeflik klasörü güncellendi',openDivisions().length+' açık bölme bulundu.','success');return true;
  }

  async function createDivision(){
    if(busy)return;
    var u=user(),input=$('seflikFolderNewBolmeV529'),bolme=clean(input&&input.value);
    if(!validIdentity(u)){notify('Kullanıcı bilgisi eksik','Önce kullanıcı panelinden ad ve şeflik kaydedin.','warning');return}
    ...
    try{
      ...
      await syncFolder(false);
      var select=$('seflikFolderBolmeV528');if(select){select.value=bolme;localStorage.setItem(LAST_BOLME_KEY,bolme)}
      updatePreview();
      setStatus('Bölme '+bolme+' ortak klasörde açıldı.','success');
      notify(out.duplicate?'Bölme zaten açıktı':'Bölme oluşturuldu','Bölme '+bolme+' artık senkronizasyona açık.','success');
  ...
  async function driveBackupCustom(list,bolme){
    ...
    if(bridge&&typeof bridge.backupCustom==='function')return bridge.backupCustom({records:list,bolmeNo:bolme,type:'mesaha-seflik-folder-backup',silent:true,sharedFolder:true});
    var u=user(),payload={type:'mesaha-seflik-folder-backup',exportedAt:new Date().toISOString(),version:(window.MESAHA_VERSION&&window.MESAHA_VERSION.version)||'local',visibleVersion:(window.MESAHA_VERSION&&window.MESAHA_VERSION.visibleVersion)||'Mesaha İO',user:u,settings:Object.assign({},settings(),{bolmeNo:bolme}),records:list,sharedFolder:true};
    var data={secret:DRIVE_SECRET,action:'backup',userKey:(fold(u.name)+'__'+fold(u.seflik)).replace(/[^a-z0-9_-]+/g,'_'),username:u.name,seflik:u.seflik,bolme:bolme,count:list.length,totalVolume:list.reduce(function(a,r){return a+volume(r)},0),version:payload.visibleVersion,payload:payload,folderMode:'seflik'};
    if(bridge&&typeof bridge.post==='function')return bridge.post('backup',data);
    var res=await fetch(DRIVE_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(data),redirect:'follow',cache:'no-store'}),txt=await res.text(),j={};try{j=txt?JSON.parse(txt):{}}catch(e){throw new Error('Drive cevabı okunamadı')}if(!res.ok||!j.ok)throw new Error(j.error||('Drive hata '+res.status));return j;
  }
  ...
  async function sendToDivision(bolme,list,origin){
    ...
    if(!openDivisions().some(function(x){return clean(x.bolme_no)===bolme})){notify('Bölme açık değil','Önce Şeflik Klasöründen bölme oluşturun.','warning');return false}
    ...
    busy=true;updatePreview();setStatus('Sunucu yüklemesi hazırlanıyor…','busy');setProgress(1);var syncToken=uuid(),drive=null,driveError='';
    try{
      for(var i=0;i<list.length;i+=CHUNK_SIZE){
        ...
        setProgress(Math.min(78,Math.round(((i+chunk.length)/list.length)*78)));setStatus('Sunucuya gönderiliyor… '+Math.min(i+chunk.length,list.length)+' / '+list.length,'busy');
      }
      setStatus('Google Drive yedeği alınıyor…','busy');setProgress(82);
      ...
      localStorage.setItem(LAST_BOLME_KEY,bolme);await persistActiveBolme(bolme);setProgress(100);await syncFolder(false);
      closeSendModal();
    ...
    finally{busy=false;modalBusy=false;updatePreview();setProgress(0);var confirmBtn=$('seflikSendConfirmV529');if(confirmBtn)confirmBtn.disabled=false}
  }
  ...
  async function recordsSendClick(){
    ...
    try{await syncFolder(false)}catch(e){notify('Bölmeler alınamadı',String(e&&e.message?e.message:e),'error');return}
    if(!openDivisions().length){notify('Önce bölme oluşturun','Şefliğe göndermek için Şeflik Klasöründe açık bir bölme oluşturulmalı.','warning');if(confirm('Şeflik Klasörüne gidip bölme oluşturmak ister misiniz?')&&typeof window.showView==='function')window.showView('seflikFolder');return}
    openSendModal();
  }
  ...
  function bind(){
    ...
    var create=$('seflikFolderCreateV529');if(create)create.addEventListener('click',createDivision);
    var newInput=$('seflikFolderNewBolmeV529');if(newInput)newInput.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();createDivision()}});
    ...
    var list=$('seflikFolderListV528');if(list)list.addEventListener('click',function(ev){var read=ev.target.closest('[data-seflik-read]');if(read){openDivision(read.getAttribute('data-seflik-read'));return}var cont=ev.target.closest('[data-seflik-continue]');if(cont)continueDivision(cont.getAttribute('data-seflik-continue'))});
    ...
    fillIdentity();var cache=readJson(CACHE_KEY,null);if(cache&&cache.seflik===user().seflik&&Array.isArray(cache.divisions))divisions=cache.divisions;renderList();
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind,{once:true});else bind();
  window.MesahaSeflikFolderV529={sync:syncFolder,send:sendFolder,sendToDivision:sendToDivision,createDivision:createDivision,openDivision:openDivision,continueDivision:continueDivision,getDivisions:function(){return divisions.slice()}};
})();

(function(){
  'use strict';
  if(window.__mesahaFilterCutterFixV455) return;
  window.__mesahaFilterCutterFixV455 = true;
  ...
  function commitSettings(st){
    ...
    dispatch('mesaha:settings-saved',{source:'v455-filter-cutter-fix'});
  }
  ...
  function renderFilters(){
    ...
    try{
      ...
      try{ if(window.MesahaErrorLog&&window.MesahaErrorLog.error) window.MesahaErrorLog.error('v455-filter-render',e); }catch(_){}
    }
    rendering=false;
  }
  ...
  function boot(){
    ...
    var rec=$('recordsView');
    if(rec && window.MutationObserver && !rec.__v455FilterWatch){
      rec.__v455FilterWatch=true;

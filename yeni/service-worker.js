importScripts('./js/version.js?v=383');

const META = self.MESAHA_VERSION || {"app": "V3.70", "version": "v383-offline-cache-guvenli", "build": 383, "visibleVersion": "V3.70 •ExelanceX•", "shortVersion": "V3.70 •ExelanceX•", "name": "Mesaha İO V3.70 •ExelanceX•", "cacheName": "mesaha-app-v383-offline-cache-guvenli", "builtAt": "2026-06-25T23:30:00+03:00", "notes": "Offline cache mimarisi shell/asset olarak ayrıldı; güncelleme ve beyaz sayfa riski azaltıldı.", "assetVersion": "383"};
...
const SHELL_ASSETS = [
  ...
  './js/version.js?v=383', './js/mesaha-early-optimizer.js?v=383', './js/mesaha-stability-core.js?v=383',
  './js/mesaha-firebase.js?v=383', './js/mesaha-offline-core.js?v=383', './js/mesaha-render-storage.js?v=383', './js/mesaha-sound.js?v=383'
];
const STATIC_ASSETS = [
...
async function put(cacheName, request, response){ try{ if(response && response.ok){ const c=await caches.open(cacheName); await c.put(request, response.clone()); } }catch(e){} }
...
async function precache(){
  ...
  await Promise.allSettled(SHELL_ASSETS.map(async u=>{ try{ const r=await fetch(u, {cache:'reload'}); if(r&&r.ok) await shell.put(u,r.clone()); }catch(e){} }));
  await Promise.allSettled(STATIC_ASSETS.map(async u=>{ try{ const old=await assets.match(u, {ignoreSearch:true}); if(old) return; const r=await fetch(u, {cache:'reload'}); if(r&&r.ok) await assets.put(u,r.clone()); }catch(e){} }));
}
...
self.addEventListener('fetch', event => {
  ...
  if(event.request.mode==='navigate'){
    event.respondWith((async()=>{
      ...
      if(cached){ event.waitUntil(networkWithTimeout(event.request,{cache:'no-store'},OFFLINE_TIMEOUT_MS).then(r=>put(SHELL_CACHE,fallback,r)).catch(()=>{})); return cached; }
      try{ const r=await networkWithTimeout(event.request,{cache:'no-store'},OFFLINE_TIMEOUT_MS); event.waitUntil(put(SHELL_CACHE,fallback,r.clone())); return r; }catch(e){ return await matchAny(event.request,fallback) || offlineHtml(); }
    })()); return;
  }
  ...
  if(isShell){ event.respondWith((async()=>{ try{ const r=await networkWithTimeout(event.request,{cache:'no-store'},OFFLINE_TIMEOUT_MS); event.waitUntil(put(SHELL_CACHE,event.request,r.clone())); return r; }catch(e){ return await matchAny(event.request, path.endsWith('/version.json')?'./version.json':null) || (isVersion?offlineJson():Response.error()); } })()); return; }
  event.respondWith((async()=>{
    const cached=await caches.match(event.request,{ignoreSearch:true}); if(cached){ event.waitUntil(networkWithTimeout(event.request,{},OFFLINE_TIMEOUT_MS).then(r=>put(ASSET_CACHE,event.request,r)).catch(()=>{})); return cached; }
    try{ const r=await networkWithTimeout(event.request,{},OFFLINE_TIMEOUT_MS); event.waitUntil(put(ASSET_CACHE,event.request,r.clone())); return r; }catch(e){ return Response.error(); }
  })());
});

...
(function(){
  ...
  function logoUrl(){try{return new URL('../assets/mesaha_logo.png',SCRIPT_SRC).href}catch(e){return '../assets/mesaha_logo.png'}}
  ...
  async function timedFetch(url,options,timeoutMs){options=options||{};var ctrl=!options.signal&&typeof AbortController!=='undefined'?new AbortController():null,timer=ctrl?setTimeout(function(){try{ctrl.abort()}catch(e){}},Math.max(1000,Number(timeoutMs)||15000)):0;try{return await fetch(url,Object.assign({},options,ctrl?{signal:ctrl.signal}:{}))}catch(e){if(e&&e.name==='AbortError')throw new Error('Sunucu bağlantısı zaman aşımına uğradı.');throw e}finally{if(timer)clearTimeout(timer)}}
  ...
  async function anonRpc(name,params){
    ...
    var res=await timedFetch(c.url+'/rest/v1/rpc/'+encodeURIComponent(name),{method:'POST',cache:'no-store',headers:{apikey:c.anonKey,Authorization:'Bearer '+c.anonKey,'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify(params||{})},15000);
  ...
  async function guestSessionPingV63(term){
    term=term||{};if(clean(term.source)==='pair_code'||navigator.onLine===false)return false;
    var c=cfg();if(!c.url||!c.anonKey)return false;
    var device={};try{device=typeof deviceInfo==='function'?deviceInfo():{}}catch(e){}
    var deviceId=clean(term.deviceId||term.device_id||device.deviceId||device.device_id);if(!deviceId){try{deviceId=localStorage.getItem('mesaha_suite_device_v7')||localStorage.getItem('mesaha_supabase_v500_device')||''}catch(e){}}
    var stampKey='mesaha_guest_admin_ping_v63_'+clean(deviceId||term.name||'local').replace(/[^a-z0-9_-]+/gi,'').slice(0,80);try{var old=Number(localStorage.getItem(stampKey)||0)||0;if(old&&Date.now()-old<20*60*1000)return false}catch(e){}
    try{
      var res=await timedFetch(c.url+'/functions/v1/smooth-function',{method:'POST',cache:'no-store',headers:{apikey:c.anonKey,Authorization:'Bearer '+c.anonKey,'Content-Type':'application/json'},body:JSON.stringify({action:'guest_session_ping',name:clean(term.name)||'Misafir kullanıcı',seflik:clean(term.seflik)||'Şeflik seçilmedi',bolmeNo:clean(term.bolmeNo||term.bolme_no),deviceId:deviceId,appVersion:clean((window.MESAHA_VERSION||{}).version||(window.MESAHA_VERSION||{}).visibleVersion||'Orman İO'),source:'manual',deviceInfo:{deviceId:deviceId,platform:(/android/i.test(navigator.userAgent||'')?'Android':/iphone|ipad|ipod/i.test(navigator.userAgent||'')?'iOS':/windows/i.test(navigator.userAgent||'')?'Windows':/macintosh|mac os/i.test(navigator.userAgent||'')?'macOS':navigator.platform||''),browser:(/samsungbrowser/i.test(navigator.userAgent||'')?'Samsung Internet':/edg\//i.test(navigator.userAgent||'')?'Edge':/crios|chrome/i.test(navigator.userAgent||'')?'Chrome':/firefox/i.test(navigator.userAgent||'')?'Firefox':/safari/i.test(navigator.userAgent||'')?'Safari':''),userAgent:navigator.userAgent||'',appId:location.pathname.indexOf('/mesaha')>=0?'mesaha':'suite',auth_type:'guest',is_local:true}})},10000);
      if(res.ok){try{localStorage.setItem(stampKey,String(Date.now()))}catch(e){}return true}
    }catch(e){}
    return false;
  }
  function session(){var a=api(),s=a&&a.getStoredSession?a.getStoredSession():null;if(s&&s.access_token)return s;try{var b=getJson(SESSION_BACKUP_KEY,null);if(b&&b.access_token){localStorage.setItem(SESSION_KEY,JSON.stringify(b));return b}}catch(e){}return null}
  ...
  function terminalData(){var x=getJson(TERMINAL_MODE_KEY,{}),li=localIdentity();return{name:clean(x.name||li.name),seflik:clean(x.seflik||li.seflik),bolmeNo:clean(x.bolmeNo||x.bolme_no||li.bolmeNo),active:x.active===true,createdAt:x.createdAt||'',source:clean(x.source||''),cloudEnabled:x.cloudEnabled===true||clean(x.source)==='pair_code'||!!x.pairedUserId,terminalCode:clean(x.terminalCode||x.code||x.p_terminal_code),terminalToken:clean(x.terminalToken||x.token||x.terminal_token),pairedUserId:clean(x.pairedUserId||x.owner_user_id),pairedEmail:clean(x.pairedEmail||x.owner_email),expiresAt:x.expiresAt||x.expires_at||'',pairedAt:x.pairedAt||x.used_at||'',avatarUrl:clean(x.avatarUrl||x.avatar_url),deviceId:clean(x.deviceId||x.device_id||x.terminalDeviceId)}}
  function writeTerminalIdentity(data){
    ...
    var avatar=clean(data.avatarUrl||data.avatar_url||data.googleAvatarUrl||data.google_avatar_url),pairedEmail=clean(data.pairedEmail||data.owner_email||''),pairedId=clean(data.pairedUserId||data.owner_user_id||'');var panel=getJson(PANEL_KEY,{});panel=Object.assign({},panel,{name:name,seflik:seflik,bolmeNo:bolme,terminalMode:true,googleApproved:false,terminalPairedUserId:pairedId,terminalPairedEmail:pairedEmail,googleUserId:pairedId||panel.googleUserId||'',googleEmail:pairedEmail||panel.googleEmail||'',googleFullName:name,googleAvatarUrl:avatar||panel.googleAvatarUrl||'',avatarUrl:avatar||panel.avatarUrl||'',updatedAt:new Date().toISOString()});setJson(PANEL_KEY,panel);
    ...
    if(data.terminalCode)term.terminalCode=clean(data.terminalCode);if(data.terminalToken)term.terminalToken=clean(data.terminalToken);if(data.pairedUserId||data.owner_user_id)term.pairedUserId=clean(data.pairedUserId||data.owner_user_id);if(data.pairedEmail||data.owner_email)term.pairedEmail=clean(data.pairedEmail||data.owner_email);if(avatar)term.avatarUrl=avatar;if(data.expiresAt)term.expiresAt=data.expiresAt;if(data.pairedAt)term.pairedAt=data.pairedAt;if(data.deviceId||data.device_id||data.terminalDeviceId)term.deviceId=clean(data.deviceId||data.device_id||data.terminalDeviceId);
  ...
  async function claimTerminalCode(codeArg){
    ...
    try{
      var terminalDevice=deviceInfo();
      var out=await anonRpc('mesaha_claim_terminal_code_v557',{p_code:code,p_device_info:terminalDevice,p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'Mesaha İO'});
      var t=out.terminal||out;
      writeTerminalIdentity({name:t.name||t.owner_name||t.canonical_name,seflik:t.seflik||t.canonical_seflik,bolmeNo:t.bolme_no||t.bolmeNo,terminalCode:code,terminalToken:t.terminal_token||t.token,pairedUserId:t.owner_user_id||t.user_id,pairedEmail:t.owner_email||t.email,avatarUrl:t.avatar_url||t.avatarUrl||t.google_avatar_url||t.picture,expiresAt:t.expires_at,pairedAt:new Date().toISOString(),deviceId:terminalDevice.deviceId||terminalDevice.device_id,source:'pair_code'});
      hide();
  ...
  function approvedFromPanel(){var p=getJson(PANEL_KEY,{}),li=localIdentity();if(!p||p.googleApproved!==true)return null;var name=clean(p.googleFullName||p.name||li.name),email=clean(p.googleEmail||''),seflik=clean(p.seflik||li.seflik||'Dosya');if(!validIdentity(name,seflik))return null;return {status:'approved',name:name,canonical_name:name,seflik:seflik,canonical_seflik:seflik,email:email,user_id:clean(p.googleUserId||''),avatar_url:clean(p.googleAvatarUrl||p.avatarUrl||''),cached_at:Date.now(),source:'panel_offline_recovery_v569'} }
  ...
  function applyIdentity(x,options){
    ...
    var li=localIdentity(), name=clean(x.name||x.canonical_name||googleDisplayName()), canonicalSeflik=clean(x.seflik||x.canonical_seflik||li.seflik||'Dosya'), bolme=clean(x.bolme_no||x.bolmeNo||li.bolmeNo), gName=googleDisplayName(), gAvatar=clean(x.avatar_url||x.google_avatar_url||googleAvatarUrl());
    var st0=getJson(SETTINGS_KEY,{}), panel0=getJson(PANEL_KEY,{}), fileSeflik=clean(st0.seflik||panel0.seflik||li.seflik||canonicalSeflik||'Dosya');
    if(!validIdentity(name,fileSeflik))return false;
    identityBusy=true;var changed=false;
    try{
      var panel=getJson(PANEL_KEY,{});
      if(clean(panel.name)!==name||clean(panel.googleUserId)!==uid()||panel.googleApproved!==true)changed=true;
      panel=Object.assign({},panel,{name:name,seflik:clean(panel.seflik||fileSeflik),bolmeNo:bolme||panel.bolmeNo||'',googleUserId:uid(),googleEmail:clean(x.email||user().email),googleFullName:clean(gName||name),googleAvatarUrl:gAvatar,avatarUrl:gAvatar,googleApproved:true});setJson(PANEL_KEY,panel);
      var st=getJson(SETTINGS_KEY,{});
      if(clean(st.ekipNot)!==name||(bolme&&clean(st.bolmeNo)!==bolme))changed=true;
      st=Object.assign({},st,{ekipNot:name,seflik:clean(st.seflik||fileSeflik||'')});if(bolme)st.bolmeNo=bolme;setJson(SETTINGS_KEY,st);
      try{
        if(window.state&&window.state.settings){
          var ws=window.state.settings;
          if(clean(ws.ekipNot)!==name||(bolme&&clean(ws.bolmeNo)!==bolme))changed=true;
          ws.ekipNot=name;if(!clean(ws.seflik))ws.seflik=fileSeflik;if(bolme)ws.bolmeNo=bolme;
          if(changed&&typeof window.saveSettings==='function')window.saveSettings();
          else if(changed&&window.MesahaStorageV527&&typeof window.MesahaStorageV527.saveSettings==='function')Promise.resolve(window.MesahaStorageV527.saveSettings(ws,{reason:'google-canonical-v567'})).catch(function(){});
        }
      }catch(e){}
...
#firstLoginOverlayV321{display:none!important}.google-auth-v548{position:fixed;inset:0;z-index:2147483600;background:linear-gradient(155deg,rgba(240,253,244,.98),rgba(239,246,255,.98));display:flex;align-items:center;justify-content:center;padding:18px;font-family:inherit}.google-auth-v548.hidden{display:none!important}html[data-suite-managed="1"] #googleAuthOverlayV548.google-auth-v548:not(.hidden){display:flex!important;visibility:visible!important;pointer-events:auto!important}.google-auth-card-v548{width:min(460px,100%);max-height:calc(100dvh - 36px);overflow:auto;background:#fff;border:1px solid #dbeafe;border-radius:28px;padding:24px;box-shadow:0 30px 90px rgba(15,23,42,.22);color:#0f172a}.google-auth-brand-v548{display:flex;gap:13px;align-items:center;margin-bottom:17px}.google-auth-brand-v548 img{width:58px;height:58px;object-fit:contain}.google-auth-brand-v548 h2{font-size:23px;margin:0}.google-auth-brand-v548 p{margin:4px 0 0;color:#64748b;font-weight:700;font-size:13px}.google-auth-status-v548{border-radius:18px;padding:14px;margin:12px 0;background:#f8fafc;border:1px solid #e2e8f0;line-height:1.45;font-weight:750;font-size:14px}.google-auth-status-v548.pending{background:#fffbeb;border-color:#fde68a;color:#92400e}.google-auth-status-v548.denied{background:#fef2f2;border-color:#fecaca;color:#991b1b}.google-auth-status-v548.ok{background:#f0fdf4;border-color:#bbf7d0;color:#166534}.google-auth-btn-v548{width:100%;min-height:54px;border:0;border-radius:17px;padding:12px 15px;font-size:16px;font-weight:950;margin-top:10px;cursor:pointer}.google-auth-btn-v548.primary{background:#fff;color:#1f2937;border:1px solid #cbd5e1;box-shadow:0 8px 22px rgba(15,23,42,.09)}.google-auth-btn-v548.primary:before{content:"G";display:inline-grid;place-items:center;width:27px;height:27px;margin-right:10px;border-radius:50%;font-size:18px;color:#2563eb;background:#eff6ff}.google-auth-btn-v548.green{background:#16a34a;color:#fff}.google-auth-btn-v548.subtle{background:#f1f5f9;color:#334155}.google-auth-btn-v548.danger{background:#fff1f2;color:#be123c}.google-auth-btn-v548:disabled{opacity:.55;cursor:wait}.google-auth-fields-v548{display:grid;gap:11px;margin:14px 0}.google-auth-fields-v548 label{font-weight:900;font-size:13px}.google-auth-fields-v548 input{box-sizing:border-box;width:100%;margin-top:6px;min-height:50px;border:1px solid #cbd5e1;border-radius:15px;padding:12px;font:inherit;font-weight:800}.google-auth-email-v548{overflow-wrap:anywhere;font-size:12px;color:#475569;text-align:center;margin:10px 0}.google-auth-note-v548{font-size:12px;color:#64748b;line-height:1.45;margin:13px 0 0}.terminal-code-v557{font-size:34px;font-weight:1000;letter-spacing:3px;text-align:center;border:1px dashed #93c5fd;border-radius:18px;background:#eff6ff;color:#1d4ed8;padding:18px;margin:12px 0}.terminal-code-panel-v557{border:1px solid #bfdbfe;background:#eff6ff;border-radius:18px;padding:12px;margin:12px 0;color:#1e3a8a}.terminal-code-panel-v557 b{font-size:15px}.terminal-code-panel-v557 p{font-size:12px;line-height:1.45;margin:6px 0 10px;color:#475569;font-weight:750}.google-auth-spinner-v548{width:24px;height:24px;border:3px solid #dbeafe;border-top-color:#2563eb;border-radius:50%;animation:googleSpinV548 .8s linear infinite;margin:8px auto}@keyframes googleSpinV548{to{transform:rotate(360deg)}}';document.head.appendChild(s)}
  ...
  async function accessStatus(force){
    ...
    statusPromise=(async function(){try{var out=await rpcCompat('mesaha_google_access_status_v560','mesaha_google_access_status_v557',{p_device_info:deviceInfo(),p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'Mesaha İO',p_google_full_name:googleDisplayName()});var x=out.access||out;x.user_id=x.user_id||uid();x.email=x.email||clean(user().email);clearEmailExistsRetry();cacheAccess(x);loginLog('access_status_ok',{status:x.status,email:x.email,user_id:x.user_id},'info');return x}catch(e){loginLog('access_status_error',{error:e,message:e&&e.message},'error');var c=approvedAccess();if(c&&c.status==='approved')return c;throw e}finally{statusPromise=null}})();
  ...
  function terminalCodeHtml(code,expiresAt){return '<div class="google-auth-status-v548 ok"><b>Terminal kodu hazır</b><br>Bu kodu terminal cihazdaki <b>Terminal giriş modu</b> ekranına girin.</div><div class="terminal-code-v557" id="terminalCodeValueV557">'+esc(code)+'</div><p class="google-auth-note-v548">Kod tek kullanımlıktır ve '+esc(expiresAt||'kısa süre')+' tarihine kadar geçerlidir. Terminal bu kodla kullanıcı/şeflik kimliğine eşleşir; bulut, Drive ve Şeflik işlemleri için terminal cihazda ayrıca Google girişi gerekir.</p>'+button('terminal-code-copy','Kodu kopyala','primary')+button('terminal-code-close','Kapat','subtle')}
  async function createTerminalCode(){var x=approvedAccess();if(!x||clean(x.status)!=='approved')throw new Error('Terminal kodu oluşturmak için önce Google ile giriş yapın.');loading('Terminal kodu oluşturuluyor…');loginLog('terminal_code_create_start',{userId:uid()},'info');var out=await rpc('mesaha_create_terminal_code_v557',{p_label:'terminal',p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'Mesaha İO'});var t=out.terminal||out;lastTerminalCode=clean(t.code);loginLog('terminal_code_create_ok',{expires_at:t.expires_at},'info');show(terminalCodeHtml(lastTerminalCode,t.expires_at));setTimeout(function(){try{renderTerminalSessionsV577()}catch(e){}},250);return out}
  function terminalDeviceLabelV577(item){item=item||{};var d=item.used_device_info||item.device_info||{},ua=clean(d.userAgent||d.user_agent),os=clean(d.os||d.platform||d.deviceType||d.device_type),code=clean(item.code);var label=clean(item.label)||os||(/iPhone|iPad|iPod/i.test(ua)?'iOS terminal':/Android/i.test(ua)?'Android terminal':'Terminal cihaz');return label+(code?' • '+code:'')}
  function dateShortV577(v){try{if(!v)return'';return new Date(v).toLocaleString('tr-TR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}catch(e){return clean(v)}}
  async function listTerminalSessionsV577(){var out=await rpc('mesaha_list_terminal_sessions_v577',{});var arr=out.terminals||out.items||[];return Array.isArray(arr)?arr:[]}
  async function revokeTerminalSessionV577(code){code=clean(code);if(!code)return;if(!confirm('Bu terminal cihazın kodlu oturumu kapatılsın mı? Terminal cihaz tekrar kod girene kadar bulut ve şeflik klasörüne giremez.'))return;var safeCode=code.replace(/"/g,'\\"');var btn=document.querySelector('[data-terminal-revoke-v577="'+safeCode+'"]');if(btn){btn.disabled=true;btn.textContent='Kapatılıyor…'}await rpc('mesaha_revoke_terminal_session_v577',{p_code:code});try{if(typeof window.mesahaFloatToastV315==='function')window.mesahaFloatToastV315('Terminal çıkışı yapıldı',code,'success')}catch(e){}await renderTerminalSessionsV577()}
  async function renderTerminalSessionsV577(){var box=document.getElementById('terminalSessionListV577');if(!box)return;box.innerHTML='<div class="google-auth-note-v548">Kod ile giriş yapan cihazlar kontrol ediliyor…</div>';try{var list=await listTerminalSessionsV577();if(!list.length){box.innerHTML='<div class="google-auth-note-v548">Kod ile giriş yapan aktif terminal cihaz yok.</div>';return}box.innerHTML='<div style="display:grid;gap:8px;margin-top:10px">'+list.map(function(t){var code=esc(t.code||'');return '<div style="border:1px solid #bfdbfe;background:#fff;border-radius:14px;padding:9px;display:grid;gap:6px"><b style="font-size:13px;color:#1e3a8a">'+esc(terminalDeviceLabelV577(t))+'</b><small style="color:#64748b;font-weight:800">Giriş: '+esc(dateShortV577(t.used_at||t.paired_at))+(t.last_cloud_at?' • Son bağlantı: '+esc(dateShortV577(t.last_cloud_at)):'')+'</small><button class="btn soft full" style="background:#fee2e2!important;color:#991b1b!important;border-color:#fecaca!important" type="button" data-terminal-revoke-v577="'+code+'">Çıkış Yap</button></div>'}).join('')+'</div>';box.querySelectorAll('[data-terminal-revoke-v577]').forEach(function(b){if(!b.__rv577){b.__rv577=true;b.addEventListener('click',function(ev){ev.preventDefault();revokeTerminalSessionV577(b.getAttribute('data-terminal-revoke-v577')).catch(fail)},true)}})}catch(e){box.innerHTML='<div class="google-auth-note-v548" style="color:#991b1b">Kodlu cihazlar alınamadı: '+esc(friendlyError(e))+'</div>'}}
  function ensureTerminalCodePanel(){try{var x=approvedAccess();if(!x||clean(x.status)!=='approved')return;var card=document.querySelector('#userPanelOverlayV316 .panel-card-v316');if(!card)return;var box=document.getElementById('terminalCodePanelV557');if(!box){var before=document.getElementById('panelTelegramSectionV515');box=document.createElement('div');box.id='terminalCodePanelV557';box.className='terminal-code-panel-v557';if(before&&before.parentNode)before.parentNode.insertBefore(box,before);else card.appendChild(box)}box.innerHTML='<b>Terminal eşleştirme</b><p>Terminal cihazda Google hesabı açmadan bu kullanıcıya bağlamak için tek kullanımlık kod oluştur.</p><button class="btn primary full" id="createTerminalCodeBtnV557" type="button">Terminal kodu oluştur</button><button class="btn soft full" id="refreshTerminalSessionsV577" type="button" style="margin-top:8px">Kod ile giriş yapan cihazları yenile</button><div id="terminalSessionListV577"></div>';var btn=document.getElementById('createTerminalCodeBtnV557');if(btn&&!btn.__terminalCodeV557){btn.__terminalCodeV557=true;btn.addEventListener('click',function(ev){ev.preventDefault();createTerminalCode().catch(fail)},true)}var rf=document.getElementById('refreshTerminalSessionsV577');if(rf&&!rf.__terminalSessionsV577){rf.__terminalSessionsV577=true;rf.addEventListener('click',function(ev){ev.preventDefault();renderTerminalSessionsV577().catch(fail)},true)}renderTerminalSessionsV577().catch(function(){})}catch(e){}}
  function renderAccess(x){
    ...
    if(status==='approved'){
      if(applyIdentity(x)){hide();ensureTerminalCodePanel();[120,900,2600].forEach(function(ms){setTimeout(function(){applyIdentity(x,{silent:true})},ms)});setTimeout(function(){try{if((!window.performance||performance.now()>3000)&&window.MesahaIpV518)window.MesahaIpV518.ping('profile_saved',true)}catch(e){}},500);return}
    }
  ...
  async function handle(action){if(action==='google'){clearTerminalMode();return beginGoogle()}if(action==='terminal')return terminalForm();if(action==='terminal-save')return saveTerminalFromForm();if(action==='terminal-claim')return claimTerminalCode();if(action==='terminal-code-copy')return copyText(lastTerminalCode).then(function(){try{if(typeof window.mesahaFloatToastV315==='function')window.mesahaFloatToastV315('Kod kopyalandı',lastTerminalCode,'success')}catch(e){}});if(action==='terminal-code-close'){hide();ensureTerminalCodePanel();return true}if(action==='refresh')return boot(true);if(action==='request')return requestAccess();if(action==='request-form')return renderAccess({status:'unregistered',email:user().email});if(action==='logout')return logout()}
    ...
    if(!session()){var ca=approvedAccess(),li=localIdentity();if(ca&&clean(ca.status)==='approved'&&validIdentity(li.name,li.seflik)){currentAccess=ca;cacheAccess(ca);loginLog('boot_cached_approved_no_session',{cachedAt:ca.cached_at,reason:'offline_or_refresh_recovery_v569'},'info');applyIdentity(ca,{silent:true});hide();ensureTerminalCodePanel();return false;}loginLog('boot_no_session',{terminalAvailable:true},'info');return show('<div class="google-auth-status-v548"><b>Google ile giriş gerekli</b><br>Bulut ve ortak şeflik özellikleri için Google hesabı gerekir.</div>'+button('google','Google ile giriş yap','primary')+button('terminal','Terminal giriş modu (Misafir modu)','subtle')+'<p class="google-auth-note-v548">Terminal koduyla yerel kullanıcı kimliğine eşleşebilirsiniz; bulut ve Şeflik işlemleri için Google girişi gerekir.</p>');}

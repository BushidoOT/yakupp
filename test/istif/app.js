...
function renderNew() {
  ...
  return `${head(draft.createdAt && state.records.some((r) => r.id === draft.id) ? "İstif Düzenle" : "Yeni İstif", "İstif bilgilerini girin", { back: true })}<form id="stackForm">
    <section class="form-card card">
      ...
      ${bolmeOptions.length ? "" : '<div class="info-note"><b>' + icon("info", 21) + "</b><span>Orman İO ana menüsünde oluşturulan bölmeyi önce Offline İndir yapın. Bölme hazır olmadan ster kaydı eklenemez.</span></div>"}
...
async function saveInstitutionSettings(event) {
  ...
  if (!folder) {
    toast("Şeflik yalnızca Orman İO listesinden seçilebilir.", "bad");
    return;
  }
  state.settings = {
    ...state.settings,
...
async function saveRecord(event, draftOnly = false) {
  ...
  if (!draftOnly && !suiteReadyBolmeler.length) {
    toast(
      "Önce Orman İO ana menüsünde en az bir bölmeyi oluşturup Offline İndir yapın.",
      "bad",
    );
    return;
  }
...
async function addForesterUser() {
  throw new Error("Ormancı yönetimi yalnızca Orman İO üzerinden yapılır.");
}

...
function ensureConfirmSound() {
  ...
  try {
    ...
    try { confirmSoundAudio.load(); } catch {}
  } catch {
...
function warmConfirmSound(event) {
  ...
  try {
    ...
    if (p && typeof p.then === 'function') {
      p.then(() => { try { audio.pause(); audio.currentTime = 0; audio.muted = false; } catch {} })
        .catch(() => { try { audio.muted = false; } catch {} });
    } else {
...
function shouldPlayConfirmSound(message, type) {
  ...
  return /kaydedildi|eklendi|tamamlandı|tamamlandi|senkronize|güncellendi|guncellendi|bağlandı|baglandi|alındı|alindi|yüklendi|yuklendi|işaretlendi|isaretlendi|geri alındı|geri alindi|seçili|secili/.test(text);
}
...
['touchstart', 'pointerdown', 'mousedown', 'keydown', 'click'].forEach((evt) => {
  try { document.addEventListener(evt, warmConfirmSound, { capture: true, passive: true }); } catch {}
});
...
function icon(name, size = 24, extraClass = '') {
  const paths = {
    ...
    external: '<path d="M14 3h7v7M10 14 21 3"/><path d="M21 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h5"/>',
...
function readSharedTerminal() {
  const terminal = jsonRead(SHARED_TERMINAL_KEY, null) || jsonRead(SHARED_TERMINAL_OLD_KEY, null) || {};
  return terminal?.active ? terminal : {};
}
function isPairedTerminal() {
  const terminal = readSharedTerminal();
  return !!(terminal.active && terminal.source === 'pair_code' && (terminal.terminalCode || terminal.terminalToken || terminal.pairedUserId));
}
function hasSharedIdentity() { return !!(readSharedSession()?.access_token || readSharedTerminal().active); }
function hasSharedCloudIdentity() { return !!(readSharedSession()?.access_token || isPairedTerminal()); }
function terminalAuthPayload() {
  const terminal = readSharedTerminal();
  return isPairedTerminal() ? { terminalCode: clean(terminal.terminalCode), terminalToken: clean(terminal.terminalToken), terminalPairedUserId: clean(terminal.pairedUserId), terminalPairedEmail: clean(terminal.pairedEmail) } : {};
}
...
function hydrateLocalSharedIdentity() {
  ...
  const terminal = readSharedTerminal();
  const localSeflik = clean(active.seflik || active.name || panel.activeSeflik || access.seflik || access.canonical_seflik || panel.seflik || terminal.seflik);
  const localKey = clean(active.seflik_key || active.seflikKey || stableKey(localSeflik));
  const localName = clean(panel.googleFullName || access.name || access.canonical_name || panel.name || terminal.name || session?.user?.user_metadata?.full_name || session?.user?.user_metadata?.name);
  const avatarUrl = clean(access.avatar_url || panel.googleAvatarUrl || session?.user?.user_metadata?.avatar_url || session?.user?.user_metadata?.picture || terminal.avatarUrl || terminal.avatar_url);
  if (session?.access_token) {
    ...
    state.auth = { ...state.auth, status: isPairedTerminal() ? 'terminal' : 'guest', userId: clean(terminal.pairedUserId), email: clean(terminal.pairedEmail), name: localName || clean(terminal.name), avatarUrl: avatarUrl || '', error: '' };
...
async function edgeCall(action, payload = {}, retried = false) {
  ...
  if (!session?.access_token && !isPairedTerminal()) throw new Error('Google veya terminal kodu ile giriş gerekli.');
  const token = session?.access_token || SUPABASE_ANON_KEY;
  const response = await fetch(EDGE_URL, { method:'POST', cache:'no-store', headers:{ apikey:SUPABASE_ANON_KEY, Authorization:`Bearer ${token}`, 'Content-Type':'application/json' }, body:JSON.stringify({ action, source:'mesaha-istif-v034-suite', ...terminalPayload, ...payload }) });
  const body = await response.json().catch(() => ({}));
  if ((response.status === 401 || response.status === 403) && session?.refresh_token && !retried) { await ensureSharedSession(true); return edgeCall(action,payload,true); }
...
async function bridgeCall(action, payload = {}, retried = false) {
  ...
  if (!session?.access_token && !isPairedTerminal()) throw new Error('Google veya terminal kodu ile giriş gerekli.');
  const token = session?.access_token || SUPABASE_ANON_KEY;
  const response = await fetch(DRIVE_BRIDGE_URL, { method:'POST', cache:'no-store', headers:{ apikey:SUPABASE_ANON_KEY, Authorization:`Bearer ${token}`, 'Content-Type':'application/json' }, body:JSON.stringify({ action, source:'mesaha-istif-v034-suite', ...terminalPayload, ...payload }) });
  const body = await response.json().catch(() => ({}));
  if ((response.status === 401 || response.status === 403) && session?.refresh_token && !retried) { await ensureSharedSession(true); return bridgeCall(action,payload,true); }
...
async function syncSharedContext({ manual = false } = {}) {
  ...
  if (navigator.onLine === false) { state.auth.status = hasSharedIdentity() ? 'cached' : 'signed_out'; if (manual) toast('İnternet yok. Son şeflik ve ormancı bilgileri kullanılıyor.'); render(); return; }
  if (!hasSharedCloudIdentity()) { hydrateLocalSharedIdentity(); refreshCurrentMembers(); render(); if (manual) toast('Yerel misafir modunda bulut senkronizasyonu kapalı.'); return; }
  state.sharedSyncing = true; state.auth.status = 'syncing'; render();
  try {
    let out, session = readSharedSession();
    try { out = await bridgeCall('shared_context', { seflik: state.settings.seflik, seflikKey: state.settings.seflikKey }); }
    catch (bridgeError) {
      const folderOut = await edgeCall('seflik_folder_list_my_sefliks', { seflik: state.settings.seflik, folderSeflik: state.settings.seflik });
      const rawFolders = Array.isArray(folderOut.folders) ? folderOut.folders : [];
      const membersBySeflik = {};
      for (const folder of rawFolders) { try { const memberOut = await edgeCall('seflik_folder_list_members', { seflik: folder.seflik, folderSeflik: folder.seflik }); membersBySeflik[folder.seflik_key || stableKey(folder.seflik)] = memberOut.members || []; } catch {} }
      out = { folders: rawFolders, membersBySeflik, customForestersBySeflik: {}, access: { user_id: readSharedTerminal().pairedUserId, email: readSharedTerminal().pairedEmail, name: readSharedTerminal().name } };
    }
    const folders = (out.folders || []).map(normalizeFolder).filter(Boolean); if (!folders.length) throw new Error('Mesaha İO Şeflik Klasörü bulunamadı.');
    state.seflikler = folders; const nextMembers = {};
    Object.entries(out.membersBySeflik || {}).forEach(([key,members]) => { nextMembers[key]=(Array.isArray(members)?members:[]).map(normalizeMember).filter(Boolean); });
    folders.forEach(folder => { const aliases=seflikAliasKeys(folder.name,folder.key); const source=aliases.map(alias=>nextMembers[alias]).find(list=>Array.isArray(list)&&list.length)||nextMembers[folder.key]||[]; mirrorMembersByAliases(nextMembers,folder,source); });
    state.membersBySeflik=nextMembers;
    if (state.settings.setupComplete && state.settings.seflik) { const selected=folders.find(x=>sameSeflikLabel(x.name,state.settings.seflik)||x.key===state.settings.seflikKey); if(selected)state.settings.seflikKey=selected.key; }
    const term=readSharedTerminal(); state.auth={ status:isPairedTerminal()?'terminal':'connected', userId:clean(out.access?.user_id||out.access?.userId||session?.user?.id||term.pairedUserId), email:clean(out.access?.email||session?.user?.email||term.pairedEmail), name:clean(out.access?.name||out.access?.canonical_name||session?.user?.user_metadata?.full_name||term.name||state.auth.name), avatarUrl:clean(out.access?.avatar_url||session?.user?.user_metadata?.avatar_url||term.avatarUrl||state.auth.avatarUrl), error:'', updatedAt:new Date().toISOString() };
    refreshCurrentMembers(); await saveSettings(); if (readSharedSession()) { await refreshDriveStatus({silent:true}); await loadRemoteRecords({silent:true}); } await saveSharedCache(); if(manual)toast('Şeflikler ve kullanıcılar güncellendi.','good');
  } catch(error) { state.auth.status=hasSharedIdentity()?'cached':'signed_out'; state.auth.error=clean(error?.message||error); hydrateLocalSharedIdentity(); refreshCurrentMembers(); if(manual)toast(`Şeflik senkronu tamamlanamadı: ${state.auth.error}`,'bad'); }
  finally { state.sharedSyncing=false; render(); }
}
...
function setView(view) {
  ...
  state.view = view;
  render();
  if (view === 'settings' && navigator.onLine && hasSharedCloudIdentity()) refreshDriveStatus({ silent: true }).then(render);
...
function authSummary() {
  if (state.auth.status === 'terminal') return `${state.auth.name || 'Terminal'} kodla bağlı • Şeflik Klasörü ortak`;
  if (state.auth.status === 'guest') return 'Yerel misafir modu • Kayıtlar cihazda';
...
async function saveInstitutionSettings(event) {
  ...
  if (!folder) { toast('Şeflik yalnızca Mesaha Suite listesinden seçilebilir.', 'bad'); return; }
  state.settings = {
    ...state.settings,
    bolgeMudurlugu,
    isletmeMudurlugu,
  ...
  if (navigator.onLine && hasSharedCloudIdentity()) syncSharedContext({ manual: false });
  render();
}
...
function showOrmanciPicker() {
  ...
  if (!state.ormancilar.length) {
    showDialog(`<h3>Ormancı bulunamadı</h3><p>Bu şefliğe Suite ana menüsünden eklenen ormancılar otomatik gelir.</p><div class="dialog-actions"><button class="btn" data-dialog-close>Kapat</button><button class="btn primary" id="refreshMembersDialog">Güncelle</button></div>`);
...
function showAddOrmanciDialog(){showDialog('<h3>Suite üzerinden yönetin</h3><p>Ormancı ekleme ve çıkarma işlemleri İstif İO içinden kaldırıldı. Mesaha Suite ana menüsündeki Ormancı Yönetimi bölümünü kullanın.</p><div class="dialog-actions"><button class="btn" data-dialog-close>Kapat</button><a class="btn primary" href="../">Suite Ana Menüsü</a></div>')}
...
async function searchForesterSuggestions(){return}

async function addForesterUser(){throw new Error('Ormancı yönetimi yalnızca Mesaha Suite üzerinden yapılır.')}

async function removeForesterUser(){throw new Error('Ormancı yönetimi yalnızca Mesaha Suite üzerinden yapılır.')}
...
async function loadRemoteRecords({ silent = true } = {}) {
  if (navigator.onLine === false || !hasSharedCloudIdentity() || !state.settings.seflik) return 0;
  try {
...
async function loadRemoteThumbnails() {
  ...
  if (!nodes.length || navigator.onLine === false || !hasSharedCloudIdentity()) return;

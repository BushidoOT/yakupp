(function(){
  ...
  function moveEntrySettingsToHome(){
    ...
    if(!woodBlock && woodPanel){
      ...
      if(apply && !apply.__v190Refresh){
        apply.__v190Refresh = true;
  ...
  function patchBrand(){
    var shortV = (window.MESAHA_VERSION && window.MESAHA_VERSION.shortVersion) || 'v2.19';
    var buildV = (window.MESAHA_VERSION && window.MESAHA_VERSION.version) || 'v190';
...
(function(){
  ...
  function titleFix(){
    safe(function(){ document.title = 'v2.19'; });
    safe(function(){
      document.querySelectorAll('.app-brand-v143 .brand-copy-v143,[data-app-version-short],[data-app-version-build]').forEach(function(el){
        if(!el) return;
        if(/Mesaha\s*İ?O/i.test(el.textContent || '')) el.textContent = el.matches('[data-app-version-build], .app-brand-v143 .brand-copy-v143 span') ? 'v190' : 'v2.19';
      });
      var h = document.querySelector('.app-brand-v143 .brand-copy-v143 h1'); if(h) h.textContent = 'v2.19';
      var s = document.querySelector('.app-brand-v143 .brand-copy-v143 span'); if(s) s.textContent = 'v190';
      var splashH = document.querySelector('.mesaha-startup-logo-v178 h2'); if(splashH) splashH.textContent = 'v2.19';
    });
  }
  function navFix(){
    safe(function(){
  ...
  function forceEditRecordToClean(){
    safe(function(){
      ...
      if(typeof old !== 'function' || old.__v190CleanOpen) return;
      ...
      wrapped.__v190CleanOpen = true;
      wrapped.__v190Old = old;
      if(old.__v158Wrapped) wrapped.__v158Wrapped = true;

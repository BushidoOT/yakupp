(function(){
  ...
  function moveEntrySettingsToHome(){
    ...
    if(!card){
      ...
      card.innerHTML = '<div class="home-card-head-v143"><h3>Giriş Ayarları</h3><button type="button" id="homeOpenCleanV188">Giriş Modu ›</button></div><div class="home-entry-settings-grid-v189"></div>';
    ...
    if(!woodBlock && woodPanel){
      ...
      if(apply && !apply.__v189Refresh){
        apply.__v189Refresh = true;
  ...
  function patchBrand(){
    var shortV = (window.MESAHA_VERSION && window.MESAHA_VERSION.shortVersion) || 'v2.18';
    var buildV = (window.MESAHA_VERSION && window.MESAHA_VERSION.version) || 'v189';
...
(function(){
  ...
  function titleFix(){
    safe(function(){ document.title = 'v2.18'; });
    safe(function(){
      document.querySelectorAll('.app-brand-v143 .brand-copy-v143,[data-app-version-short],[data-app-version-build]').forEach(function(el){
        if(!el) return;
        if(/Mesaha\s*İ?O/i.test(el.textContent || '')) el.textContent = el.matches('[data-app-version-build], .app-brand-v143 .brand-copy-v143 span') ? 'v189' : 'v2.18';
      });
      var h = document.querySelector('.app-brand-v143 .brand-copy-v143 h1'); if(h) h.textContent = 'v2.18';
      var s = document.querySelector('.app-brand-v143 .brand-copy-v143 span'); if(s) s.textContent = 'v189';
      var splashH = document.querySelector('.mesaha-startup-logo-v178 h2'); if(splashH) splashH.textContent = 'v2.18';
    });
  }
  function navFix(){
    safe(function(){
  ...
  function orderFix(){
    safe(function(){
      ...
      if(top && top.parentNode === card && top.nextSibling !== measure) card.insertBefore(measure, top.nextSibling);
      else if(measure.parentNode === card && card.firstElementChild && !top) card.insertBefore(measure, save);
      ...
      if(section.parentNode !== card || section.previousElementSibling !== measure) card.insertBefore(section, save);
      var lenWrap = byId('cleanLengthV111') ? byId('cleanLengthV111').closest('div') : null;
      var diaWrap = byId('cleanDiameterV111') ? byId('cleanDiameterV111').closest('div') : null;
      if(lenWrap && diaWrap && measure.firstElementChild !== diaWrap) measure.insertBefore(diaWrap, lenWrap);
    });
  }
  function run(){ titleFix(); navFix(); orderFix(); }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', run, {once:true}); else run();
  [50,150,350,800,1500,2600,5200].forEach(function(ms){ setTimeout(run, ms); });
  setInterval(run, 900);
  window.mesahaV189GoHome = goHome;
})();

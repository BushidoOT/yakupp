(() => {
  ...
  const ADMIN_VERSION = (window.MESAHA_VERSION && window.MESAHA_VERSION.visibleVersion) || 'Mesaha İO';
...
function appLabel(user){return appKind(user)==='istif'?'İstif İO':appKind(user)==='suite'?'Orman İO':'Mesaha İO'}
  ...
  async function login(){
    const userInput=$('loginUser'),passInput=$('loginPass'),message=$('loginMessage'),button=$('loginBtn');if(!userInput||!passInput||!message||!button)return;const email=lower(userInput.value),password=clean(passInput.value);
    if(!/^\S+@\S+\.\S+$/.test(email)||!password){message.textContent='Geçerli admin e-postası ve parolayı girin.';return}
    message.textContent='Giriş doğrulanıyor…';button.disabled=true;
    try{const out=await rawEdge('admin_login',{email,password});if(!out||!out.session||!out.session.access_token)throw new Error('Yönetim oturumu oluşturulamadı');saveSession(out.session);passInput.value='';message.textContent='';showAdmin();await loadAll()}
    catch(error){clearSession();message.textContent=errorText(error)}
  ...
  function bind(){
    const versionLabel=$('versionLabel');if(versionLabel)versionLabel.textContent=ADMIN_VERSION;
    const loginButton=$('loginBtn'),loginForm=$('loginForm');if(loginForm)loginForm.addEventListener('submit',(e)=>{e.preventDefault();login()});else if(loginButton)loginButton.addEventListener('click',(e)=>{e.preventDefault();login()});

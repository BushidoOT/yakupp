...
const TABLES = {
  ...
  seflikFolders: "mesaha_seflik_folders",
  seflikMembers: "mesaha_seflik_members",
  ...
  terminalPairing: "mesaha_terminal_pairing_codes",
  userDriveBackups: "mesaha_user_drive_backups",
  istifRecords: "mesaha_istif_records",
};
...
function googleAvatarUrl(user: any): string {
  const urls: string[] = [];
  const add = (v: any) => { const t = clean(v); if (/^https?:\/\//i.test(t) && !urls.includes(t)) urls.push(t); };
  try { const m = obj(user?.user_metadata); add(m.avatar_url); add(m.picture); } catch {}
  try { const gd = obj(googleIdentity(user).identity_data); add(gd.avatar_url); add(gd.picture); } catch {}
  return clean(urls[0] || '').slice(0, 600);
}
...
function accessView(row: JsonObj | null, ctx?: any): JsonObj {
  ...
  return {
    ...
    requested_name: clean(r.requested_name), requested_seflik: clean(r.requested_seflik), reason: clean(r.reason), avatar_url: clean(r.avatar_url || obj(r.metadata).avatar_url || obj(r.metadata).google_avatar_url),
...
function terminalCodeFromBody(body: JsonObj): string {
  const raw = first(body.terminalCode, body.terminal_code, body.p_terminal_code);
  const c = clean(raw).toUpperCase().replace(/[^A-Z0-9]/g, "");
  return c.length === 8 ? `${c.slice(0, 4)}-${c.slice(4)}` : "";
}
async function terminalAccessContext(req: Request, body: JsonObj) {
  const code = terminalCodeFromBody(body);
  if (!code) return null;
  const token = first(body.terminalToken, body.terminal_token, body.p_terminal_token);
  const deviceInfo = { ...obj(body.deviceInfo), ...obj(body.device_info), ...obj(body.lastDeviceInfo) };
  const deviceId = first(body.deviceId, body.device_id, deviceInfo.deviceId, deviceInfo.device_id).slice(0, 120);
  const ipInfo = clientIpInfo(req, body);
  const pair = obj(await selectFirst(TABLES.terminalPairing, { code }));
  if (!pair.code || clean(pair.status) !== "used") {
    return { response: json({ ok: false, terminal_required: true, error: "Terminal eşleştirmesi bulunamadı. Ana cihazdan yeni kod alın veya çıkış yapıp tekrar eşleştirin." }, 403) };
  }
  const tokenHash = clean(pair.terminal_token_hash);
  if (tokenHash) {
    const incoming = await hashText(token);
    if (incoming !== tokenHash) {
      return { response: json({ ok: false, terminal_required: true, error: "Terminal güvenlik anahtarı geçersiz. Ana cihazdan çıkış yapıp yeni kodla tekrar eşleştirin." }, 403) };
    }
  } else if (clean(pair.used_by_device_id) && clean(pair.used_by_device_id) !== deviceId) {
    return { response: json({ ok: false, terminal_required: true, error: "Terminal farklı cihazdan kullanılamaz." }, 403) };
  }
  const access = obj(await selectFirst(TABLES.userAccess, { user_id: clean(pair.owner_user_id) }));
  if (!access.user_id || clean(access.status) !== "approved") {
    return { response: json({ ok: false, access_required: true, error: "Terminal sahibinin Google erişimi kapalı" }, 403) };
  }
  const userId = clean(pair.owner_user_id);
  const email = lower(pair.owner_email || access.email);
  const name = first(pair.name, access.canonical_name, access.requested_name);
  const seflik = first(pair.seflik, access.canonical_seflik, access.requested_seflik);
  const userKey = clean(access.user_key) || userKeyFrom(name, seflik);
  const profile: JsonObj = {
    user_id: userId,
    user_key: userKey || null,
    name: name || null,
    seflik: seflik || null,
    bolme_no: first(pair.bolme_no, access.bolme_no, body.bolmeNo, body.bolme_no) || null,
    app_version: first(body.appVersion, body.app_version, body.fileVersion) || null,
    device_id: deviceId || null,
    last_ip: clean(ipInfo.ip) || null,
    last_seen_at: nowIso(),
    platform: first(deviceInfo.platform, deviceInfo.os) || null,
    browser: first(deviceInfo.browser, deviceInfo.browserName) || null,
    browser_version: first(deviceInfo.browserVersion, deviceInfo.browser_version) || null,
    device_info: deviceInfo,
    payload: { ...body, edgeIp: ipInfo, terminalCode: code },
    avatar_url: clean(access.avatar_url || obj(access.metadata).avatar_url || obj(access.metadata).google_avatar_url) || null,
  };
  try { await admin.from(TABLES.terminalPairing).update({ last_cloud_at: nowIso(), used_device_info: { ...obj(pair.used_device_info), ...deviceInfo } }).eq("code", code); } catch {}
  const block = await activeBlock("user_id", userId);
  if (block) return { response: json({ ok: false, blocked: true, block_type: "user_id", reason: first(block.reason, "Yönetici engeli") }, 403) };
  return { userId, email, access, profile, ipInfo, userKey, terminal: true, terminalCode: code, user: null };
}

async function approvedAccessContext(req: Request, body: JsonObj) {
  const terminalCtx: any = await terminalAccessContext(req, body);
  if (terminalCtx) return terminalCtx;
  ...
  const avatarUrl = googleAvatarUrl(ctx.user);
  const canonicalName = first(googleName, access.canonical_name);
  const canonicalBody: JsonObj = { ...body,
    name: clean(canonicalName), googleFullName: googleName || undefined, avatar_url: avatarUrl || clean(access.avatar_url), google_avatar_url: avatarUrl || clean(access.avatar_url), seflik: clean(access.canonical_seflik),
  ...
  profile.name = clean(canonicalName); profile.seflik = clean(access.canonical_seflik); profile.user_key = clean(access.user_key); profile.avatar_url = avatarUrl || clean(access.avatar_url) || null;
...
async function userAccessRequest(req: Request, body: JsonObj) {
  ...
  const googleName = googleDisplayName(ctx.user);
  const avatarUrl = googleAvatarUrl(ctx.user);
  const name = first(googleName, body.googleFullName, body.google_full_name, body.name).slice(0, 120), seflik = first(body.seflik, body.canonical_seflik, 'Dosya').slice(0, 120);
  ...
  const row = {
    ...
    device_id: deviceId || null, device_info: device, metadata: { app_version: clean(body.appVersion), provider: "google", google_full_name: googleName || null, avatar_url: avatarUrl || null, google_avatar_url: avatarUrl || null, request_source: "edge-v564-auto-approved" }, avatar_url: avatarUrl || null,
  };
...
async function profileHeartbeat(req: Request, body: JsonObj) {
  ...
  if (ctx.response) {
    /* V5.63: Eski istemciler Google/oturum gerektiğini 403 görünce ban ekranı açıyordu.
       Profil ping ve bilinmeyen eski aksiyonlarda gerçek admin engeli yoksa 200 döndürülür. */
    const ipInfo = clientIpInfo(req, body);
    const auth = await authContext(req);
    const device = { ...obj(body.deviceInfo), ...obj(body.device_info), ...obj(body.lastDeviceInfo) };
    const user = obj(body.user);
    const name = first(body.name, user.name, body.userName, body.kullanici, device.name);
    const seflik = first(body.seflik, user.seflik, body.seflikAdi, device.seflik);
    const userKey = lower(first(body.userKey, body.user_key, user.userKey, user.user_key, name && seflik ? userKeyFrom(name, seflik) : ""));
    const deviceId = first(body.deviceId, body.device_id, device.deviceId, device.device_id);
    const block = await anyBlock({ user_id: clean(auth.userId), user_key: userKey, ip: clean(ipInfo.ip), device_id: deviceId });
    if (block) return json({ ok: false, blocked: true, block_type: clean(block.block_type), reason: first(block.reason, "Yönetici engeli") }, 403);
    return json({ ok: true, blocked: false, soft_allowed: true, google_required: true, login_required: !clean(auth.userId), ip: clean(ipInfo.ip), server_time: nowIso(), message: "Google oturumu gerekli" });
  }
...
async function driveUserAction(req: Request, body: JsonObj, action: string) {
  ...
  if (!["backup", "list", "read", "delete"].includes(normalized)) return json({ ok: false, error: "Drive işlemi desteklenmiyor" }, 400);
  const limit = normalized === "backup" ? 6 : normalized === "delete" ? 10 : 30;
  const rate = await takeRate(`drive-${normalized}:${ctx.userId}`, limit, 3600, normalized === "backup" ? 1800 : normalized === "delete" ? 600 : 300);
  ...
  if (normalized === "delete") {
    ...
    if (!fileId) return json({ ok: false, error: "Silinecek Drive yedeği seçilmedi" }, 400);

    /* Kullanıcı yalnız kendi userKey listesinde görünen Drive yedeğini silebilir. */
    const ownedList: any = await driveProxy("list", { userKey: ctx.userKey });
    const ownedItems = arr(ownedList?.items);
    const ownsFile = ownedItems.some((x) => clean(obj(x).id || obj(x).fileId || obj(x).file_id) === fileId);
    if (!ownsFile) return json({ ok: false, error: "Bu Drive yedeğini silme yetkiniz yok" }, 403);

    const driveDelete = await deleteDriveFilesPermanent([fileId]);
    const results: any[] = [];
    results.push(...(await safeDeleteMany(TABLES.backupSlots, ["drive_file_id", "file_id", "id", "slot_id"], fileId)));
    results.push(...(await safeDeleteMany(TABLES.backupChunks, ["drive_file_id", "file_id", "slot_id"], fileId)));
    try { await admin.from(TABLES.seflikSyncs).delete().eq("drive_file_id", fileId).eq("user_id", ctx.userId); } catch {}
    await insertEvent({
      user_id: ctx.userId,
      ...
      seflik: clean(ctx.profile.seflik),
      event_type: "drive_user_delete_backup",
      blocked: false,
      metadata: { file_id: fileId, drive_delete: driveDelete, supabase_deletes: results },
    });
    return json({
      ok: driveDelete.failed.length === 0,
      fileId,
      deleted: driveDelete.deleted,
      failed: driveDelete.failed,
      drive_delete: driveDelete,
      supabase_deletes: results,
    }, driveDelete.failed.length ? 502 : 200);
  }
...
function profileValues(body: JsonObj, userId: string, ipInfo: JsonObj): JsonObj {
  ...
  return {
    ...
    avatar_url: first(body.avatar_url, body.google_avatar_url, body.avatarUrl, user.avatar_url, deviceInfo.avatar_url) || null,
  };
}
...
function folderView(row: JsonObj, member?: JsonObj, ctx?: any): JsonObj {
  const r = obj(row), m = obj(member);
  return {
    id: clean(r.id), seflik_key: clean(r.seflik_key), seflik: clean(r.seflik), status: clean(r.status || 'active'),
    created_by_user_id: clean(r.created_by_user_id), created_by_name: clean(r.created_by_name), created_by_avatar_url: clean(r.created_by_avatar_url),
    role: clean(m.role || (clean(r.created_by_user_id) === clean(ctx?.userId) ? 'owner' : 'member')),
    is_creator: clean(r.created_by_user_id) === clean(ctx?.userId), created_at: r.created_at || null, updated_at: r.updated_at || null,
  };
}

async function ensureSeflikOwnerMember(folderRaw: JsonObj, ctx: any) {
  const folder = obj(folderRaw);
  if (!clean(folder.seflik_key) || !ownsLegacyFolder(folder, ctx, clean(folder.seflik_key))) return null;
  const existing = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: clean(folder.seflik_key), user_id: clean(ctx.userId) }));
  if (clean(existing.status) === 'active') return existing;
  const avatar = googleAvatarUrl(ctx.user) || clean(ctx.profile.avatar_url);
  const mem = {
    id: `${clean(folder.seflik_key)}__${clean(ctx.userId)}`,
    seflik_key: clean(folder.seflik_key),
    user_id: clean(ctx.userId),
    user_key: clean(ctx.userKey) || null,
    email: lower(ctx.email),
    name: clean(ctx.profile.name || folder.created_by_name),
    avatar_url: avatar || clean(folder.created_by_avatar_url) || null,
    role: 'owner',
    status: 'active',
    added_by_user_id: clean(ctx.userId),
    created_at: nowIso(),
    updated_at: nowIso(),
  };
  try { await admin.from(TABLES.seflikMembers).upsert(mem, { onConflict: 'seflik_key,user_id' }); } catch {}
  try { if (clean(folder.created_by_user_id) !== clean(ctx.userId)) await admin.from(TABLES.seflikFolders).update({ created_by_user_id: clean(ctx.userId), created_by_user_key: clean(ctx.userKey) || null, updated_at: nowIso() }).eq('seflik_key', clean(folder.seflik_key)); } catch {}
  return mem;
}
function ownsLegacyFolder(folderRaw: JsonObj, ctx: any, requestedKey = '') {
  const f = obj(folderRaw);
  if (clean(f.created_by_user_id) === clean(ctx.userId)) return true;
  if (clean(f.created_by_user_key) && clean(f.created_by_user_key) === clean(ctx.userKey)) return true;
  if (requestedKey && clean(f.seflik_key) === requestedKey && clean(f.created_by_name) && clean(f.created_by_name) === clean(ctx.profile.name)) return true;
  return false;
}
async function seflikFolderListMySefliks(req: Request, body: JsonObj) {
  const ctx: any = await approvedAccessContext(req, body);
  if (ctx.response) return ctx.response;
  const requestedSeflik = first(body.folderSeflik, body.seflikFolder, body.seflik, ctx.profile.seflik).slice(0, 120);
  const requestedKey = stableFolderKey(requestedSeflik, 160);
  if (requestedSeflik && requestedKey) {
    try { await ensureActiveSeflikForUser(ctx, requestedSeflik, true); } catch { /* Listeleme, onarım başarısız olsa da devam eder. */ }
  }
  const memberships = await selectAllPaged(TABLES.seflikMembers, { user_id: ctx.userId, status: 'active' }, 'created_at', 2000);
  if (memberships.error && isMissingTable(memberships.error)) return json({ ok: true, folders: [], missing_sql: true, error: 'Şeflik üyelik tabloları kurulmamış. V5.64 SQL çalıştırılmalı.' });
  const out: JsonObj[] = [];
  const seen = new Set<string>();
  for (const memRaw of memberships.items || []) {
    const mem = obj(memRaw), key = clean(mem.seflik_key);
    if (!key || seen.has(key)) continue;
    seen.add(key);
    const f = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: key }));
    if (clean(f.status || 'active') !== 'active') continue;
    out.push(folderView(f, mem, ctx));
  }
  const created = await selectAllPaged(TABLES.seflikFolders, { created_by_user_id: ctx.userId, status: 'active' }, 'updated_at', 2000);
  for (const fRaw of created.items || []) {
    const f = obj(fRaw), key = clean(f.seflik_key);
    if (!key || seen.has(key)) continue;
    const mem = await ensureSeflikOwnerMember(f, ctx) || { role: 'owner' };
    seen.add(key); out.push(folderView(f, mem, ctx));
  }
  if (requestedKey && !seen.has(requestedKey)) {
    const f = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: requestedKey }));
    if (f.id && clean(f.status || 'active') === 'active') {
      const mem = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: requestedKey, user_id: ctx.userId }));
      if (clean(mem.status) === 'active' || ownsLegacyFolder(f, ctx, requestedKey)) {
        const fixedMem = clean(mem.status) === 'active' ? mem : await ensureSeflikOwnerMember(f, ctx) || { role: ownsLegacyFolder(f, ctx, requestedKey) ? 'owner' : 'member' };
        seen.add(requestedKey); out.push(folderView(f, fixedMem, ctx));
      }
    }
  }
  return json({ ok: true, folders: out.sort((a,b)=>clean(a.seflik).localeCompare(clean(b.seflik), 'tr')), active_seflik: requestedSeflik || null, server_time: nowIso() });
}
async function ensureActiveSeflikForUser(ctx: any, requestedSeflikRaw: unknown, allowCreate = true) {
  const seflik = first(requestedSeflikRaw, ctx?.profile?.seflik).slice(0, 120);
  const seflikKey = stableFolderKey(seflik, 160);
  if (!seflik || !seflikKey) return { ok: false, allowed: false, error: 'Geçerli şeflik adı gerekli', folder: null, member: null };
  const avatar = googleAvatarUrl(ctx.user) || clean(ctx.profile.avatar_url);
  let folder = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: seflikKey }));
  let created = false, repaired = false;

  if (!folder.id || clean(folder.status || '') === 'deleted') {
    if (!allowCreate) return { ok: false, allowed: false, error: 'Şeflik klasörü bulunamadı', folder: null, member: null, seflik, seflikKey };
    const alreadyCreated = await selectAllPaged(TABLES.seflikFolders, { created_by_user_id: ctx.userId, status: 'active' }, 'updated_at', 20);
    const ownActiveFolder = (alreadyCreated.items || []).map((x) => obj(x)).find((x) => clean(x.seflik_key));
    if (ownActiveFolder && clean(ownActiveFolder.seflik_key) !== seflikKey) {
      const ownMem = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: clean(ownActiveFolder.seflik_key), user_id: ctx.userId }));
      return { ok: true, allowed: true, created: false, repaired: false, folder: ownActiveFolder, member: ownMem, seflik: clean(ownActiveFolder.seflik), seflikKey: clean(ownActiveFolder.seflik_key), note: 'Kullanıcının mevcut kurduğu şeflik döndürüldü' };
    }
    const row = {
      id: seflikKey,
      seflik_key: seflikKey,
      seflik,
      status: 'active',
      created_by_user_id: ctx.userId,
      created_by_user_key: ctx.userKey || null,
      created_by_name: clean(ctx.profile.name),
      created_by_avatar_url: avatar || null,
      created_at: nowIso(),
      updated_at: nowIso(),
    };
    const { data, error } = await admin.from(TABLES.seflikFolders).upsert(row, { onConflict: 'id' }).select('*').limit(1);
    if (error) return { ok: false, allowed: false, error: isMissingTable(error.message) ? 'Şeflik klasörü tablosu kurulmamış. SQL migration çalıştırılmalı.' : error.message, folder: null, member: null, seflik, seflikKey };
    folder = obj(arr(data)[0] || row);
    created = true;
  } else if (clean(folder.status || 'active') !== 'active') {
    const { data, error } = await admin.from(TABLES.seflikFolders).update({ status: 'active', updated_at: nowIso() }).eq('seflik_key', seflikKey).select('*').limit(1);
    if (!error) { folder = obj(arr(data)[0] || folder); repaired = true; }
  }

  let member = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: seflikKey, user_id: ctx.userId }));
  const isCreator = clean(folder.created_by_user_id) === clean(ctx.userId) || ownsLegacyFolder(folder, ctx, seflikKey) || created;
  if (isCreator && clean(member.status) !== 'active') {
    const fixed = await ensureSeflikOwnerMember(folder, ctx);
    if (fixed) { member = obj(fixed); repaired = true; }
  }
  if (clean(member.status) === 'active') return { ok: true, allowed: true, created, repaired, folder, member, seflik: clean(folder.seflik || seflik), seflikKey };
  if (isCreator) return { ok: true, allowed: true, created, repaired, folder, member: { role: 'owner', status: 'active' }, seflik: clean(folder.seflik || seflik), seflikKey };
  return { ok: false, allowed: false, created, repaired, folder, member, seflik: clean(folder.seflik || seflik), seflikKey, error: 'Bu şeflik klasörünü sadece içindeki kullanıcılar kullanabilir' };
}

async function seflikFolderEnsureActive(req: Request, body: JsonObj) {
  const ctx: any = await approvedAccessContext(req, body);
  if (ctx.response) return ctx.response;
  const requestedSeflik = first(body.folderSeflik, body.seflikFolder, body.seflik, ctx.profile.seflik).slice(0, 120);
  const ensured: any = await ensureActiveSeflikForUser(ctx, requestedSeflik, true);
  if (!ensured.ok || !ensured.folder) return json({ ok: false, error: ensured.error || 'Şeflik klasörü hazırlanamadı' }, ensured.allowed === false ? 403 : 500);
  const folder = folderView(obj(ensured.folder), obj(ensured.member), ctx);
  return json({ ok: true, folder, member: obj(ensured.member), seflik: clean(folder.seflik), seflik_key: clean(folder.seflik_key), is_creator: !!folder.is_creator, created: !!ensured.created, repaired: !!ensured.repaired, server_time: nowIso() });
}

async function seflikFolderCreateSeflik(req: Request, body: JsonObj) {
  const ctx: any = await approvedAccessContext(req, body);
  if (ctx.response) return ctx.response;
  const seflik = first(body.seflik, body.folderSeflik, body.seflik_name).slice(0, 120);
  const seflikKey = stableFolderKey(seflik, 160);
  if (seflik.length < 2 || !seflikKey) return json({ ok: false, error: 'Geçerli şeflik adı gerekli' }, 400);
  const existing = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: seflikKey }));
  if (clean(existing.status || '') === 'active') {
    const mem = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: seflikKey, user_id: ctx.userId }));
    if (clean(existing.created_by_user_id) === ctx.userId || clean(mem.status) === 'active') return json({ ok: true, duplicate: true, folder: folderView(existing, mem, ctx) });
    return json({ ok: false, error: 'Bu şeflik zaten oluşturulmuş. Kurucu silmeden yeniden oluşturulamaz.' }, 409);
  }
  const alreadyCreated = await selectAllPaged(TABLES.seflikFolders, { created_by_user_id: ctx.userId, status: 'active' }, 'updated_at', 20);
  const ownActiveFolder = (alreadyCreated.items || []).map((x) => obj(x)).find((x) => clean(x.seflik_key));
  if (ownActiveFolder && clean(ownActiveFolder.seflik_key) !== seflikKey) {
    return json({ ok: false, error: 'Bir kullanıcı sadece 1 şeflik kurabilir. Yeni şeflik kurmak için önce kurduğunuz şefliği silin.', existing_folder: folderView(ownActiveFolder, { role: 'owner' }, ctx) }, 409);
  }
  const avatar = googleAvatarUrl(ctx.user) || clean(ctx.profile.avatar_url);
  const row = { id: seflikKey, seflik_key: seflikKey, seflik, status: 'active', created_by_user_id: ctx.userId, created_by_user_key: ctx.userKey || null, created_by_name: clean(ctx.profile.name), created_by_avatar_url: avatar || null, created_at: nowIso(), updated_at: nowIso() };
  const { data, error } = await admin.from(TABLES.seflikFolders).upsert(row, { onConflict: 'id' }).select('*').limit(1);
  if (error) return json({ ok: false, error: isMissingTable(error.message) ? 'Şeflik üyelik tablosu kurulmamış. V5.64 SQL çalıştırılmalı.' : error.message }, isMissingTable(error.message) ? 503 : 500);
  const folder = obj(arr(data)[0] || row);
  const mem = { id: `${seflikKey}__${ctx.userId}`, seflik_key: seflikKey, user_id: ctx.userId, user_key: ctx.userKey || null, email: lower(ctx.email), name: clean(ctx.profile.name), avatar_url: avatar || null, role: 'owner', status: 'active', added_by_user_id: ctx.userId, created_at: nowIso(), updated_at: nowIso() };
  await admin.from(TABLES.seflikMembers).upsert(mem, { onConflict: 'seflik_key,user_id' });
...
async function seflikFolderDeleteSeflik(req: Request, body: JsonObj) {
  const ctx: any = await approvedAccessContext(req, body);
  if (ctx.response) return ctx.response;
  const seflik = first(body.seflik, body.folderSeflik).slice(0, 120), confirm = first(body.confirmSeflik, body.confirm_seflik, body.seflik).slice(0, 120);
  const key = stableFolderKey(seflik, 160);
  if (!key || confirm !== seflik) return json({ ok: false, error: 'Şeflik silme onayı eşleşmiyor' }, 400);
  const folder = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: key }));
  if (!folder.id || clean(folder.status || 'active') !== 'active') return json({ ok: false, error: 'Silinecek şeflik bulunamadı' }, 404);
  if (clean(folder.created_by_user_id) !== ctx.userId) return json({ ok: false, error: 'Bu şefliği sadece oluşturan kullanıcı silebilir' }, 403);
  const results: JsonObj[] = [];
  results.push(await safeDeleteFilters(TABLES.seflikRecords, { seflik_key: key }));
  results.push(await safeDeleteFilters(TABLES.seflikSyncs, { seflik_key: key }));
  results.push(await safeDeleteFilters(TABLES.seflikDivisions, { seflik_key: key }));
  results.push(await safeDeleteFilters(TABLES.seflikMembers, { seflik_key: key }));
  await admin.from(TABLES.seflikFolders).update({ status: 'deleted', deleted_at: nowIso(), deleted_by_user_id: ctx.userId, updated_at: nowIso() }).eq('seflik_key', key);
  await insertEvent({ user_id: ctx.userId, ip_address: clean(ctx.ipInfo.ip) || null, user_key: ctx.userKey || null, name: clean(ctx.profile.name), seflik, event_type: 'seflik_folder_delete_seflik', blocked: false, metadata: { seflik_key: key, results } });
  return json({ ok: true, seflik, seflik_key: key, results });
}
async function seflikFolderSearchUsers(req: Request, body: JsonObj) {
  const ctx: any = await seflikFolderContext(req, body);
  if (ctx.response) return ctx.response;
  const q = first(body.query, body.name).slice(0, 80);
  if (q.length < 3) return json({ ok: false, error: 'Arama için ad soyadın en az 3 harfi gerekli' }, 400);
  const safeQ = q.replace(/[%_,]/g,'');
  let query: any = admin.from(TABLES.userAccess).select('user_id,email,canonical_name,requested_name,avatar_url,metadata,status').eq('status', 'approved').limit(20);
  query = query.or(`canonical_name.ilike.%${safeQ}%,requested_name.ilike.%${safeQ}%,email.ilike.%${safeQ}%`).order('canonical_name', { ascending: true, nullsFirst: false });
  const { data, error } = await query;
  if (error) return json({ ok: false, error: error.message }, 500);
  const users = arr(data).map((x) => obj(x)).filter((u) => clean(u.user_id) !== ctx.userId).map((u) => ({ user_id: clean(u.user_id), email: lower(u.email), name: first(u.canonical_name, u.requested_name), avatar_url: clean(u.avatar_url || obj(u.metadata).avatar_url || obj(u.metadata).google_avatar_url) }));
  return json({ ok: true, users });
}
async function seflikFolderAddMember(req: Request, body: JsonObj) {
  const ctx: any = await seflikFolderContext(req, body);
  if (ctx.response) return ctx.response;
  if (!ctx.folder || clean(ctx.folder.created_by_user_id) !== ctx.userId) return json({ ok: false, error: 'Ormancı eklemeyi sadece şefliği oluşturan kullanıcı yapabilir' }, 403);
  const memberUserId = first(body.member_user_id, body.user_id);
  if (!memberUserId) return json({ ok: false, error: 'Eklenecek kullanıcı seçilmedi' }, 400);
  const access = obj(await selectFirst(TABLES.userAccess, { user_id: memberUserId }));
  if (!access.user_id || clean(access.status) !== 'approved') return json({ ok: false, error: 'Kullanıcı bulunamadı veya Google ile onaylı değil' }, 404);
  const row = { id: `${ctx.seflikKey}__${memberUserId}`, seflik_key: ctx.seflikKey, user_id: memberUserId, user_key: clean(access.user_key) || null, email: lower(access.email), name: first(access.canonical_name, access.requested_name), avatar_url: clean(access.avatar_url || obj(access.metadata).avatar_url || obj(access.metadata).google_avatar_url) || null, role: 'member', status: 'active', added_by_user_id: ctx.userId, created_at: nowIso(), updated_at: nowIso() };
  const { data, error } = await admin.from(TABLES.seflikMembers).upsert(row, { onConflict: 'seflik_key,user_id' }).select('*').limit(1);
  if (error) return json({ ok: false, error: error.message }, 500);
  await insertEvent({ user_id: ctx.userId, ip_address: clean(ctx.ipInfo.ip) || null, user_key: ctx.userKey || null, name: clean(ctx.profile.name), seflik: ctx.seflik, event_type: 'seflik_folder_add_member', blocked: false, metadata: row });
  return json({ ok: true, member: arr(data)[0] || row });
}

async function seflikFolderListMembers(req: Request, body: JsonObj) {
  const ctx: any = await seflikFolderContext(req, body);
  if (ctx.response) return ctx.response;
  let { data, error } = await admin.from(TABLES.seflikMembers)
    .select('id,seflik_key,user_id,user_key,email,name,avatar_url,role,status,added_by_user_id,created_at,updated_at')
    .eq('seflik_key', ctx.seflikKey)
    .eq('status', 'active')
    .order('role', { ascending: false })
    .order('name', { ascending: true });
  if (error) return json({ ok: false, error: error.message }, 500);
  const creator = clean(ctx.folder.created_by_user_id) === ctx.userId || ownsLegacyFolder(ctx.folder, ctx, ctx.seflikKey);
  let rawMembers = arr(data).map((x) => obj(x));
  if (creator && !rawMembers.some((m) => clean(m.user_id) === ctx.userId && clean(m.role) === 'owner')) {
    const mem = await ensureSeflikOwnerMember(ctx.folder, ctx);
    if (mem) rawMembers.unshift(obj(mem));
  }
  const members = rawMembers.map((m) => ({
    id: clean(m.id), seflik_key: clean(m.seflik_key), user_id: clean(m.user_id), user_key: clean(m.user_key),
    email: lower(m.email), name: clean(m.name), avatar_url: clean(m.avatar_url), role: clean(m.role || 'member'),
    status: clean(m.status || 'active'), is_self: clean(m.user_id) === ctx.userId, can_remove: creator && clean(m.user_id) !== ctx.userId && clean(m.role) !== 'owner',
    created_at: m.created_at || null, updated_at: m.updated_at || null,
  }));
  return json({ ok: true, seflik: ctx.seflik, seflik_key: ctx.seflikKey, is_creator: creator, members });
}
async function seflikFolderRemoveMember(req: Request, body: JsonObj) {
  const ctx: any = await seflikFolderContext(req, body);
  if (ctx.response) return ctx.response;
  if (!ctx.folder || clean(ctx.folder.created_by_user_id) !== ctx.userId) return json({ ok: false, error: 'Ormancı çıkarmayı sadece şefliği oluşturan kullanıcı yapabilir' }, 403);
  const memberUserId = first(body.member_user_id, body.user_id);
  if (!memberUserId) return json({ ok: false, error: 'Çıkarılacak kullanıcı seçilmedi' }, 400);
  if (memberUserId === ctx.userId) return json({ ok: false, error: 'Kurucu kendisini çıkaramaz. Şefliği silmek için Şefliği Sil butonunu kullanın.' }, 400);
  const member = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: ctx.seflikKey, user_id: memberUserId }));
  if (!member.id || clean(member.status) !== 'active') return json({ ok: false, error: 'Ekli ormancı bulunamadı' }, 404);
  if (clean(member.role) === 'owner') return json({ ok: false, error: 'Kurucu üyelik çıkarılamaz. Şefliği silmek gerekir.' }, 400);
  const { error } = await admin.from(TABLES.seflikMembers).update({ status: 'removed', updated_at: nowIso() }).eq('seflik_key', ctx.seflikKey).eq('user_id', memberUserId);
  if (error) return json({ ok: false, error: error.message }, 500);
  await insertEvent({ user_id: ctx.userId, ip_address: clean(ctx.ipInfo.ip) || null, user_key: ctx.userKey || null, name: clean(ctx.profile.name), seflik: ctx.seflik, event_type: 'seflik_folder_remove_member', blocked: false, metadata: { seflik_key: ctx.seflikKey, member_user_id: memberUserId, member_name: clean(member.name), member_email: lower(member.email) } });
  return json({ ok: true, removed: true, seflik: ctx.seflik, seflik_key: ctx.seflikKey, member_user_id: memberUserId });
}
...
async function seflikFolderContext(req: Request, body: JsonObj) {
  ...
  if (ctx.response) return ctx;
  const requestedSeflik = first(body.folderSeflik, body.seflikFolder, body.seflik, ctx.profile.seflik).slice(0, 120);
  const seflikKey = stableFolderKey(requestedSeflik, 160);
  if (!seflikKey) return { response: json({ ok: false, error: "Önce Şeflik Klasörü içinde şeflik oluşturun veya seçin" }, 400) };
  let folder = obj(await selectFirst(TABLES.seflikFolders, { seflik_key: seflikKey }));
  if (!folder.id || clean(folder.status || 'active') !== 'active') {
    const ensured: any = await ensureActiveSeflikForUser(ctx, requestedSeflik, true);
    if (!ensured.ok || !ensured.folder || !ensured.allowed) return { response: json({ ok: false, error: ensured.error || "Bu şeflik klasörü oluşturulmamış. Önce Şeflik Oluştur kullanın." }, ensured.allowed === false ? 403 : 404) };
    folder = obj(ensured.folder);
  }
  let member = obj(await selectFirst(TABLES.seflikMembers, { seflik_key: seflikKey, user_id: ctx.userId }));
  const isCreator = clean(folder.created_by_user_id) === ctx.userId || ownsLegacyFolder(folder, ctx, seflikKey);
  if (isCreator && clean(member.status) !== 'active') member = obj(await ensureSeflikOwnerMember(folder, ctx) || member);
  if (!isCreator && clean(member.status) !== 'active') return { response: json({ ok: false, error: "Bu şeflik klasörünü sadece içindeki kullanıcılar kullanabilir" }, 403) };
  const seflik = clean(folder.seflik || requestedSeflik);
  return { ...ctx, seflik, seflikKey, folder, member: isCreator ? { ...member, role: clean(member.role || 'owner'), status: 'active' } : member };
}
...
async function seflikFolderPush(req: Request, body: JsonObj) {
  ...
  /* Bölme içindeki kayıt kimliği kullanıcıya değil barkoda bağlıdır. Böylece farklı
     kullanıcıların aynı barkodu ikinci kez göndermesi kopya üretmez; son kayıt güncellenir. */
  const uniqueRows = new Map<string, JsonObj>();
  for (const r of input) {
    const recordKey = stableFolderKey(first(r.barcode, r.barkodNo, r.barkod_no, r.id), 180);
    if (!recordKey) continue;
    const barcode = first(r.barcode, r.barkodNo, r.barkod_no, r.id);
    const rowId = stableFolderKey(`${ctx.seflikKey}__${bolmeKey}__${recordKey}`, 480);
    uniqueRows.set(recordKey, {
      ...
      record_data: { ...r, bolmeNo, bolme_no: bolmeNo, seflik: ctx.seflik },
  ...
  return json({ ok: true, accepted: rows.length, mergedByBarcode: true, syncToken, bolmeNo });
}
async function seflikFolderFinish(req: Request, body: JsonObj) {
  ...
  /* V17: Gönderimler snapshot değiştirmez; mevcut bölmenin devamına barkoda göre eklenir. */
  const removedOldRecords = 0;
...
async function seflikFolderList(req: Request, body: JsonObj) {
  ...
  const completedTokens = new Set<string>((syncResult.items || []).map((row) => clean(row.sync_token)).filter(Boolean));

  /* V17: Tüm tamamlanmış gönderimler bölmenin devamıdır. Barkod bazında yalnızca
     en güncel satır hesaba katılır; farklı kullanıcıların kayıtları birlikte görünür. */
  const latest = new Map<string, JsonObj>();
  for (const row of recResult.items) {
    const token = clean(row.sync_token);
    if (token && !completedTokens.has(token)) continue;
    const bolmeKey = clean(row.bolme_key);
    const recordKey = clean(row.record_key);
    if (!bolmeKey || !recordKey) continue;
    const key = `${bolmeKey}|${recordKey}`;
...
async function istifRecordUpsert(req: Request, body: JsonObj) {
  const ctx: any = await seflikFolderContext(req, body);
  if (ctx.response) return ctx.response;
  const r = obj(body.record || body.data);
  const id = first(r.id, r.record_id).slice(0, 240);
  const bolmeNo = first(r.bolme_no, r.bolmeNo, r.bolme).slice(0, 120);
  if (!id || !bolmeNo) return json({ ok: false, error: 'İstif kayıt kimliği ve bölme zorunlu' }, 400);
  const division = await seflikFolderDivision(ctx, bolmeNo);
  if (!division.row || clean(division.row.status || 'open') !== 'open') return json({ ok: false, error: 'Bu bölme sunucuda açık değil. Önce bölmeyi senkronize edin.' }, 409);
  const row = {
    id,
    user_id: ctx.userId,
    seflik_key: ctx.seflikKey,
    seflik: ctx.seflik,
    ormanci: first(r.ormanci, r.forester, ctx.profile.name) || null,
    record_date: first(r.record_date, r.date, trDate()),
    bolme_no: bolmeNo,
    istif_no: first(r.istif_no, r.istifNo) || null,
    wood_type: first(r.wood_type, r.type) || null,
    ster: Number(num(r.ster).toFixed(3)),
    coordinates: first(r.coordinates, r.coordinate, r.kordinat) || null,
    mevki: first(r.mevki, r.location_note) || null,
    description: first(r.description, r.aciklama) || null,
    barcode_no: first(r.barcode_no, r.barcode) || null,
    photo_count: Math.max(0, Math.round(num(r.photo_count || r.photoCount))),
    drive_folder_id: first(r.drive_folder_id, r.driveFolderId) || null,
    drive_files: Array.isArray(r.drive_files) ? r.drive_files : Array.isArray(r.driveFiles) ? r.driveFiles : [],
    sync_status: 'synced',
    is_sent: bool(r.is_sent ?? r.isSent),
    sent_at: first(r.sent_at, r.sentAt) || null,
    sent_by: first(r.sent_by, r.sentBy) || null,
    created_at: first(r.created_at, r.createdAt, nowIso()),
    updated_at: nowIso(),
  };
  const { data, error } = await admin.from(TABLES.istifRecords).upsert(row, { onConflict: 'id' }).select('*').limit(1);
  if (error) return json({ ok: false, error: error.message }, isMissingTable(error.message) ? 503 : 500);
  await admin.from(TABLES.seflikDivisions).update({ last_activity_at: nowIso(), updated_at: nowIso() }).eq('seflik_key', ctx.seflikKey).eq('bolme_key', stableFolderKey(bolmeNo, 120));
  await insertEvent({ user_id: ctx.userId, user_key: ctx.userKey || null, name: clean(ctx.profile.name), seflik: ctx.seflik, event_type: 'istif_record_upsert', blocked: false, metadata: { id, bolme_no: bolmeNo, istif_no: row.istif_no, ster: row.ster, photo_count: row.photo_count } });
  return json({ ok: true, record: arr(data)[0] || row });
}

async function adminListAll(body: JsonObj) {
  const limit = Math.min(Math.max(Number(body.limit || 2500) || 2500, 100), 5000);
  const [profiles0, usage, backups, logs, daily, events, blocks, adminAuditRows, userAccessRows, userAuthEvents, seflikFoldersRows, seflikMembersRows, seflikDivisionsRows, userDriveBackupRows] = await Promise.all([
    ...
    selectRows(TABLES.userDriveBackups, { order: "created_at", limit: 5000 }),
  ]);
  ...
  return json({
    ...
    backups: [...arr(backups.items), ...arr(userDriveBackupRows.items).map((x) => ({ ...obj(x), provider: "user-drive-v7", readonly_admin: true }))],
    logs: logs.items,
    events: events.items,
    blocks: blocks.items,
    ...
    seflik_folders: seflikFoldersRows.items,
    ...
    errors: {
      ...
      user_drive_backups: userDriveBackupRows.error || null,
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (action.startsWith("seflik_folder_")) {
      ...
      if (action === "seflik_folder_list_members") return await seflikFolderListMembers(req, body);
      if (action === "seflik_folder_remove_member") return await seflikFolderRemoveMember(req, body);
    ...
    if (action === "istif_record_upsert") return await istifRecordUpsert(req, body);
    ...
    if (action === "drive_backup" || action === "drive_list" || action === "drive_read" || action === "drive_delete") return await driveUserAction(req, body, action);

-- Mesaha İO V5.59 — Terminal kodu ile bulut yedek erişimi
-- V5.57 migration'ı çalıştırılmış olmalıdır.

begin;

create extension if not exists pgcrypto;

alter table public.mesaha_terminal_pairing_codes
...
begin
  ...
  update public.mesaha_terminal_pairing_codes
     set status = 'used',
         ...
         metadata = coalesce(metadata,'{}'::jsonb) || jsonb_build_object('claimed_app_version', nullif(btrim(coalesce(p_app_version,'')), ''), 'cloud_enabled', true)
  ...
  values(v_row.owner_user_id, v_row.owner_email, 'terminal_code_claimed_rpc_v559_cloud_enabled', true, nullif(v_device_id,''), jsonb_build_object('code', v_code_fmt, 'name', v_row.name, 'seflik', v_row.seflik), now());
...
begin
  ...
  v_count := greatest(0, coalesce((p_meta->>'recordCount')::integer, (p_meta->>'count')::integer, 0));
  v_total := coalesce((p_meta->>'totalVolume')::numeric, (p_meta->>'m3')::numeric, 0);
  ...
  values(v_owner.owner_user_id, v_slot, v_owner.user_key, v_owner.name, v_owner.seflik, v_name, v_count, v_total, v_payload, false, 'terminal-cloud-v559')
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'chunkCount', coalesce((s.payload->>'chunkCount')::integer, 1),
    'raw', s.payload

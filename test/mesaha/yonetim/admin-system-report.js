...
(function () {
  ...
  function summaryCards(summary) {
    return `<div class="system-report-grid">
      ...
      <article class="system-report-card ${Number(summary.pending_sync || 0) ? "warn" : ""}"><small>Bekleyen senkron</small><b>${formatNumber(summary.pending_sync)}</b><span>İstif, Suite ve manuel Mesaha</span></article>
  ...
  function driveHtml(rows) {
    ...
    if (!list.length) return '<div class="system-report-empty">Drive alan raporu henüz gelmedi. Kullanıcı Suite ana ekranını açtığında otomatik gelir.</div>';

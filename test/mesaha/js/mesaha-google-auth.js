...
(function(){
  ...
  async function claimTerminalCode(codeArg){
    ...
    try{
      var out=await anonRpc('mesaha_claim_terminal_code_v557',{p_code:code,p_device_info:deviceInfo(),p_app_version:(window.MESAHA_VERSION||{}).visibleVersion||'Mesaha İO'});
      var t=out.terminal||out;
      writeTerminalIdentity({name:t.name||t.owner_name||t.canonical_name,seflik:t.seflik||t.canonical_seflik,bolmeNo:t.bolme_no||t.bolmeNo,terminalCode:code,terminalToken:t.terminal_token||t.token,pairedUserId:t.owner_user_id||t.user_id,pairedEmail:t.owner_email||t.email,avatarUrl:t.avatar_url||t.avatarUrl||t.google_avatar_url||t.picture,expiresAt:t.expires_at,pairedAt:new Date().toISOString(),source:'pair_code'});
      hide();
